...
static const ACodecPLL_APP ACodecpllTable_61440[8]=
{
    ...
    #else
    {24000 ,49152 , 0  ,192,125, 10},//PLL_OUTDIV = 6
    #endif
...
static const ACodecPLL_APP ACodecpllTable_56448[8]=
{
    ...
    #else
    {24000 ,45158 , 0  ,147, 125, 3},//PLL_OUTDIV = 5
    #endif
...
static const ACodecPLL_APP ACodecpllTable_40960[8]=
{
    ...
    #else
    {24000 ,32384 , 6   ,189,125, 8},//PLL_OUTDIV = 9
    #endif
...
void ACodec_PLL_Set(eF_SOURCE_ID F_SOURCE,eACodecPll_Target_Freq pll_target)
{
    ...
    #else
    WriteAcodecReg(ACODEC_BASE+ACODEC_PLLCFG4,(OUTDIV << 4) | 4);
    #endif
...
#if(VOLTAB_CONFIG == VOL_General)
_ATTR_FMCONTROL_TEXT_ FM_SRUCT_CODEC_CONFIG FmCodecConfig_General[MAX_VOLUME+1] =
{
...
#else //(VOLTAB_CONFIG == VOL_General)

_ATTR_FMCONTROL_TEXT_  FM_SRUCT_CODEC_CONFIG FmCodecConfig_Europe[MAX_VOLUME+1] =
{
     15, 0, 12,   0, 254, 15,  //  0
     15, 0, 12,   0, 160, 15,  //  1
     15, 0, 12,   0, 148, 15,  //  2
...
{
    SRUCT_CODEC_CONFIG    *pAudioVolTab;
    FM_SRUCT_CODEC_CONFIG *pFMVolTab;
    ...
    {
        #if(VOLTAB_CONFIG == VOL_General)
        pFMVolTab = (FM_SRUCT_CODEC_CONFIG *)FmCodecConfig_General;
        #else
        pFMVolTab = (FM_SRUCT_CODEC_CONFIG *)FmCodecConfig_Europe;
        #endif

        if (Volume == 0)
        {
            ACodec_Set_ADC_DigVol(pFMVolTab[Volume].ADCDigVol);
            ACodec_Set_ADCMUX_Vol(pFMVolTab[Volume].ADCMuxVol);
            ACodec_Set_HP_AMP(pFMVolTab[Volume].HP_AMPVol);
            ACodec_Set_DAC_DigVol(pFMVolTab[Volume].Dac_DigVol);
            ACodec_Set_HP_Gain(pFMVolTab[Volume].HP_ANTIPOPVol);
        }
        else
        {
            ACodec_Set_ADC_DigVol(pFMVolTab[Volume].ADCDigVol);
    ...
    {
        #if(VOLTAB_CONFIG == VOL_General)
        pAudioVolTab = (SRUCT_CODEC_CONFIG *)ACodec_HPoutVol_General;
        #else
        pAudioVolTab = (SRUCT_CODEC_CONFIG *)CodecConfig_Europe;
        #endif

        if (Volume == 0)
        {
            {
                ACodec_Set_DAC_DigVol(pAudioVolTab[Volume].Dac_DigVol);
                ACodec_Set_HP_Gain(pAudioVolTab[Volume].HP_ANTIPOPVol);
                ACodec_Set_HP_AMP(pAudioVolTab[Volume].HP_AMPVol);
            }
            Codec_DACMute();
        }
        else
        {
            ...
            switch (VolumeMode)
            {
                ...
                case EQ_HEAVY:
                    {
                        {
                            ACodec_Set_DAC_DigVol(pAudioVolTab[Volume].EQ_POP_HEAVY_DacDigVol);
                            ACodec_Set_HP_Gain(pAudioVolTab[Volume].HP_ANTIPOPVol);
                            ACodec_Set_HP_AMP(pAudioVolTab[Volume].EQ_POP_HEAVY_HP_AMPVol);
                        }
                    }
                    break;
                ...
                case EQ_UNIQUE:
                    {
                        {
                            ACodec_Set_DAC_DigVol(pAudioVolTab[Volume].EQ_JAZZ_UNIQUE_DacDigVol);
                            ACodec_Set_HP_Gain(pAudioVolTab[Volume].HP_ANTIPOPVol);
                            ACodec_Set_HP_AMP(pAudioVolTab[Volume].EQ_JAZZ_UNIQUE_HP_AMPVol);
                        }
                    }
                    break;
                ...
                case EQ_USER:
                    {
                        {
                            ACodec_Set_DAC_DigVol(pAudioVolTab[Volume].EQ_USER_DacDigVol);
                            ACodec_Set_HP_Gain(pAudioVolTab[Volume].HP_ANTIPOPVol);
                            ACodec_Set_HP_AMP(pAudioVolTab[Volume].EQ_USER_HP_AMPVol);
                        }
                    }
                    break;
                ...
                case EQ_BASS:
                    {
                        {
                            ACodec_Set_DAC_DigVol(pAudioVolTab[Volume].EQ_BASS_DacDigVol);
                            ACodec_Set_HP_Gain(pAudioVolTab[Volume].HP_ANTIPOPVol);
                            ACodec_Set_HP_AMP(pAudioVolTab[Volume].EQ_BASS_HP_AMPVol);
                        }
                    }
