   ...
   UINT32 RealFreq, Source_clk,div_num,ret;
    ...
    if(I2sId == I2S_CH1)
    {
        ...
        else if(Clk_Source == CLK_ACODEC_PLL)
        {
            ...
            #else
            // 4分频，固定值，因为考虑到ACODEC PLL使用与I2S0与I2S1，所以I2S1外置的MCLK CLOCK分频保持跟ACODEC内部一致4分频
            // 采用外接ACODEC并且作为Master时，I2S需要偶数分频
            CRU->CRU_CLKSEL_CON[3] = ((CLK_SEL_MASK_7F << 16) | 3) << 0;
            #endif
    ...
    else if(I2sId == I2S_CH0)
    {
        ...
        else if(Clk_Source == CLK_ACODEC_PLL)
        {
            ...
            // 4分频，固定值，因为考虑到ACODEC PLL使用与I2S0与I2S1，所以I2S1外置的MCLK CLOCK分频保持跟ACODEC内部一致4分频
            // 采用外接ACODEC并且作为Master时，I2S需要偶数分频
            CRU->CRU_CLKSEL_CON[4] = ((CLK_SEL_MASK_7F << 16) | 3) << 0;
            #endif
