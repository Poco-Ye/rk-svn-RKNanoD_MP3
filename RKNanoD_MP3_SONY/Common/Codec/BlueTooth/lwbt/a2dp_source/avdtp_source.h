...
#define AVDTP_SEP_ID_SOUCRE        0x01
#define AVDTP_SEP_ID_SINK          0x02
...
__packed struct get_configuration_cmd
{
    uint8 MPT;
    uint8 SIRFA;
    uint8 Acp_SEID;
...
__packed struct avdtp_general_reject
{
    uint8 error_code;
};

__packed struct avdtp_config_reject
{
    uint8 service_category;
    uint8 error_code;
};

#define AVDTP_SEP_MAX 2
__packed struct a2dp_pcb
{

    struct a2dp_pcb *next; /* For the linked list */
    struct avdtp_local_sep local_sep[AVDTP_SEP_MAX];
    struct Local_SBC_CODEC_info Localcodec[AVDTP_SEP_MAX];
...
//  struct l2cap_pcb *l2cappcb_sign; /* The L2CAP connection */
    uint8  RemoteSEIDCnt;
    uint8  role; /*avdtp role when = 0 is source, when =1 is sink */
    uint8  select_seid;
