...
_ATTR_LWBT_DATA_
static uint8 avrcp_service_record_ct[] =
{
    0x35, 0x08,
    0x09 ,0x00 ,0x00 ,0x0a ,0x00 ,0x01 ,0x00 ,0x02 ,
    0x35, 0x08,
    0x09 ,0x00 ,0x01 ,0x35 ,0x03 ,0x19 ,0x11 ,0x0F ,
    0x35, 0x08,
    0x09 ,0x00 ,0x02 ,0x0a ,0x00 ,0x00 ,0x00 ,0x06 ,
    0x35, 0x15,
    0x09 ,0x00 ,0x04 ,0x35 ,0x10 ,0x35 ,0x06 ,0x19 ,0x01 ,0x00 ,0x09 ,0x00 ,0x17 ,0x35 ,0x06 ,
    ...
    0x74 ,0x20 ,0x4f ,0x6e ,0x65 ,
    0x35, 0x6,
    0x09 ,0x03 ,0x11 ,0x09 ,0x00 ,0x02 ,0x0
};
...
_ATTR_LWBT_INIT_CODE_
void avctp_audio_play_notify(void(*audio_play_notify)(int button_status))
{
    avctp_notify_pcb.audio_play= audio_play_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_stop_notify(void(*audio_stop_notify)(int button_status))
{
    avctp_notify_pcb.audio_stop= audio_stop_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_pause_notify(void(*audio_pause_notify)(int button_status))
{
...
_ATTR_LWBT_INIT_CODE_
void avctp_audio_next_notify(void(*audio_next_notify)(int button_status))
{
    avctp_notify_pcb.audio_next= audio_next_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_previous_notify(void(*audio_previous_notify)(int button_status))
{
    avctp_notify_pcb.audio_previous= audio_previous_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_fastforward_notify(void(*audio_fastforward_notify)(int button_status))
{
    avctp_notify_pcb.audio_ffw= audio_fastforward_notify;
}


_ATTR_LWBT_INIT_CODE_
void avctp_audio_rewind_notify(void(*audio_rewind_notify)(int button_status))
{
    avctp_notify_pcb.audio_ffd= audio_rewind_notify;
}



_ATTR_LWBT_INIT_CODE_
void avctp_audio_mute_notify(void(*audio_mute_notify)(int button_status))
{
    avctp_notify_pcb.audio_mute= audio_mute_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_volumeup_notify(void(*audio_volumeup_notify)(int button_status))
{
    avctp_notify_pcb.audio_volumeup= audio_volumeup_notify;
}

_ATTR_LWBT_INIT_CODE_
void avctp_audio_volumedown_notify(void(*audio_volumedown_notify)(int button_status))
{
...
void avctp_init(void)
{
    ...
    if((record = sdp_record_new((uint8 *)avrcp_service_record_ct, sizeof(avrcp_service_record_ct))) == NULL)
    {
        LWBT_DEBUGF(AVCTP_DEBUG, _DBG_INFO_, ("Could not alloc SDP record ct\n"));
        //rk_printf("Could not alloc SDP record ct");
        //return ERR_MEM;
    }
    else
    {
        //rk_printf("sdp_register_service _avctp ct");
        sdp_register_service(record);
    }
...
err_t avctp_l2cap_connected_ind(void *arg, struct l2cap_pcb *pcb, err_t err)
{
    ...
    LWBT_DEBUGF(AVCTP_DEBUG, _DBG_WARNING_, ("avctp_l2cap_connected_ind: L2CAP connected\n"));
...
_ATTR_LWBT_CODE_
static int avctp_rsp_invalid_pid(struct avctp_header *avctp_in)
{
    int ret;
    ...
    send_pbuf  = pbuf_alloc(PBUF_RAW, AVCTP_HEADER_LENGTH,PBUF_RAM);
    if (send_pbuf == NULL)
        return FALSE;
...
_ATTR_LWBT_CODE_
bool avctp_passthrough_rsp(struct avctp_pcb *session,uint8 code,uint8 *operands)
{
    DEBUG("avctp passthrougth respond.");

    if(code != AVC_CTYPE_ACCEPTED)
        return FALSE;

    //judge whether key pressed have released.
    if( operands[0] & 0x80 )
        return FALSE;
...
_ATTR_LWBT_CODE_
bool avctp_command_process(struct avctp_pcb *session,uint8 code,uint8 *operands,int operand_count)
{
    uint8 op;
...
//        return TRUE;
//    }//  20161230@wp release command must need by audio_ffd and audio_ffw
    ...
    switch( op )
    {
        case AVC_PLAY:
            if(avctp_notify_pcb.audio_play != NULL)
                avctp_notify_pcb.audio_play(operands[0] >> 7);
            break;

        case AVC_STOP:
            if(avctp_notify_pcb.audio_stop!= NULL)
                avctp_notify_pcb.audio_stop(operands[0] >> 7);
            break;

        case AVC_PAUSE:
            if(avctp_notify_pcb.audio_pause!= NULL)
                avctp_notify_pcb.audio_pause(operands[0] >> 7);
        ...
        case AVC_REWIND:
            if(avctp_notify_pcb.audio_ffd!= NULL)
                avctp_notify_pcb.audio_ffd(operands[0] >> 7);
            break;

        case AVC_FAST_FORWARD:

            if(avctp_notify_pcb.audio_ffw!= NULL)
                avctp_notify_pcb.audio_ffw(operands[0] >> 7);
        ...
        case AVC_FORWARD:
            ...
            if(avctp_notify_pcb.audio_previous!= NULL)
                avctp_notify_pcb.audio_previous(operands[0] >> 7);
        ...
        case AVC_BACKWARD:
            ...
            if(avctp_notify_pcb.audio_next!= NULL)
                avctp_notify_pcb.audio_next(operands[0] >> 7);
...
//  packet_size = AVCTP_HEADER_LENGTH + AVC_HEADER_LENGTH;
    ...
    if( avctp_hdr->cr == AVCTP_RESPONSE)
    {
        ...
        if(avctp_hdr->pid != htons( AV_REMOTE_SVCLASS_ID ))
        {
            ...
            avctp_rsp_invalid_pid(avctp_hdr);
            goto done;
        }
        else //is the avctp profile.
        {
            ...
            switch( avc_hdr->opcode )
            {
                ...
                case AVC_OP_PASSTHROUGH:
                    avctp_passthrough_rsp(avctppcb,code,operands);
                    break;
                default:
                    break;
            }
    ...
    else if( avctp_hdr->cr == AVCTP_COMMAND )
    {
        ...
        else //is the avctp profile.
        {
            ...
            {
                case AVC_OP_PASSTHROUGH:
                    bret = avctp_command_process(avctppcb,code,operands,operand_count);
                    break;
