...
void blueToothConnectReqProc(struct bd_addr *bdaddr)
{
    ...
    //hci_accept_connection_Request(bdaddr, 0x01); //所有设备都可以连接    不允许切换角色
	  hci_accept_connection_Request(bdaddr, 0x00); //所有设备都可以连接    允许切换角色
#else
...
void audio_get_media_attribute_title(uint8* buf ,int inputlen, int * outlen)
{
    ...
    else if (TRUE == ThreadCheck(pMainThread, &MusicThread))
    {
        ...
        uint32 unicodeLen = 0;
        uint16 *unicodeData;
        //char testbuf[780]; //for pts test
        memset(buf, 0 , inputlen);
        unicodeData = (uint16 *)&gAudioId3Info.id3_title[0];

        //memset(testbuf, 0x0a, 768);//for pts test
        //unicodeData = (uint16 *)testbuf;//for pts test
