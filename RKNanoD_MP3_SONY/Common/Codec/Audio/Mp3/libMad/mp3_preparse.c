    ...
    if (firstheaderposition)
    {
        ...
        if (!returnvalue)
        {
            ...
            if (vbr_flag)
            {
                if ((firstframe - 1024) > 0)
                {
                    firstframe = firstframe - 1024 + i - 36;
                }
                else
                {
                    firstframe = i - 36;
...
//              if(result<=0) return -1;
            ...
            if (returnvalue)
            {
                ...
                if ( (ulFirstHeaderInfo & 0x001E0D03) == (ulSecondHeaderInfo & 0x001E0D03) )
                {
                    ...
                    // mp3_printf("\n文件差12 =%d \n",pMP3->ulFirstFrame);
#if 1
//重新修正比特率，否则部分歌曲的时间和比特率显示不准
                    // check 20 header's bitrate ===================================================
                    ulBitrateCount = 0;

                    for (comparecount = 0; comparecount < 10; comparecount++)
                    {
                        // jump file position
                        MP3Data = (unsigned char*) pENCODED_DATA;
...
//          if(result<=0) return -1;
    ...
    mp3_printf("\n return here means get head position fail.\n");
