...
#if defined(OPT_SSO)
#define SHIFT(x)  (((x) + (1L << 11)) >> 12) //>>13bit没有溢出
#else
#define SHIFT(x)  (x)
#endif
...
#undef MUL
#undef SHIFT/* third SSO shift and/or D[] optimization preshift */
#define OPT_SSO
...
#endif
#define ML0(hi, lo, x, y)	((lo)  = (long long)(x) * (long long)(y))
#define MLA(hi, lo, x, y)	((lo) += (long long)(x) * (long long)(y))
#define MLN(hi, lo)		((lo)  = -(lo))
//#define MLZ(hi, lo)		((void) (hi), (mad_fixed_t) (lo))
#define MLZ(hi, lo)		((void) (hi), (lo))
#define SHIFT(x)		((x) >> 2)
#define PRESHIFT(x)		((MAD_F(x) + (1L << 13)) >> 14)
#else
#define ML0(hi, lo, x, y)	MAD_F_ML0((hi), (lo), (x), (y))
#define MLA(hi, lo, x, y)	MAD_F_MLA((hi), (lo), (long long)(x),(long long)(y))
		...
		unsigned int nch, unsigned int ns)
{
  ...
  //mp3_printf("synth_full synth_full synth_full");
  for (ch = 0; ch < nch; ++ch) {//channels
    sbsample = &frame->sbsample[ch];
    filter   = &synth->filter[ch];
    phase    = synth->phase;
    pcm1     = synth->pcm.samples[ch];
   // mp3_printf("unsigned long long =%d ",sizeof(unsigned long long));
    // mp3_printf("mad_fixed64lo_t =%d ",sizeof(long));
    for (s = 0; s < ns; ++s) {//36 samples
...
void mad_synth_frame(struct mad_synth *synth, struct mad_frame const *frame)
{
  ...
  synth->pcm.channels   = nch;
  synth->pcm.length     = 32 * ns;
 // mp3_printf("ns = %d,length = %d",ns,synth->pcm.length);
/* {
	  int i;
	  int j;
	  for (i = 0; i < 36;i++)
		 for (j = 0; j < 32; j++)
		{
			 mp3_printf("%d %d", (int)((frame->sbsample[0][i][j]>>32)&0xffffffffL),(int)(frame->sbsample[0][i][j]&0xffffffffL));//第0个子带有溢出。。。
			// mp3_printf("%d ", );
			 //printf("!!!!");
  ...
  /*{
    ...
    for(i=0;i<32;i++)
      for(j=0;j<36;j++)
        mp3_printf("sbsample[%d][%d]= %d",j,i,*frame->sbsample[j][i]);
...
/*  {
     int i = 0;
     mad_fixed_t *pcm;
     pcm = synth->pcm.samples[0];
     for(i=0;i<1152;i++)
         mp3_printf("samples[%d]=%d\n",i,pcm[i]);
