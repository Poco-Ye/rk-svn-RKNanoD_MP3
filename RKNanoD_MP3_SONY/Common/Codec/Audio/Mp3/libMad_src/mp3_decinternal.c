  ...
  //  mp3_printf("nchannels = %d ",nchannels);
   // mp3_printf("nsamples = %d ",nsamples);
        ...
        }
    {
       ...
       // mp3_printf("0x%x",(unsigned long)should_out);
	}
...
int mp3_get_buffer(short** ulParam1 , int *ulParam2)
{
    ...
    *(int *)ulParam2 = outlength;
    //mp3_printf("shout_out = %x ,len = %d \n",(unsigned long)should_out1,outlength);
    return 1;
}
...
int mp3_decode()
{
    ...
    do
    {
        //mp3_printf("flag_fail = %d",flag_fail);
        if (flag_fail == 1)
            goto decode_fail;
