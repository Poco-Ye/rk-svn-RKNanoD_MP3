...
unsigned long IMAADPCM_FORMAT(tPCM *pPCM)
{
	...
	decode_ima_adpcm((char*)pPCM->pucEncodedData + pPCM->usOffset,
					...
					(short*)(&pPCM->OutputBuff[pPCM->CurDecodeIdx][SRC_Num_Forehead]),//WAV_IMAMAX_PCM_LENGTH
...
#if 0
{
		...
		for (i = 0 ; i < 3 ; i ++ )
		{
			decode_ima_adpcm((char*)pPCM->pucEncodedData + pPCM->usOffset,
		                (long int)lBlockSize*4,
										(short*)(&pPCM->OutputBuff[pPCM->CurDecodeIdx][generatedSampleSize * i]),//WAV_IMAMAX_PCM_LENGTH
...

	{
		...
		pO = &pPCM->OutputBuff[pPCM->CurDecodeIdx][SRC_Num_Forehead];
		pL = &pPCM->OutputBuff[pPCM->CurDecodeIdx][SRC_Num_Forehead];
		pR = &ima_tmp[0];
