...
static  void _sbc_process_4bytes(const uint32 *in, uint32 *out)
{
    ...
    cut[0] = SCALE4_STAGE2( s[0] + s[1] + s[2] - s[4]); /* Q0 */
    cut[1] = SCALE4_STAGE2(-s[0] + s[1] + s[3]);
    cut[2] = SCALE4_STAGE2(-s[0] + s[1] - s[3]);
    cut[3] = SCALE4_STAGE2( s[0] + s[1] - s[2] + s[4]);
    for(i = 0;i < 4;i ++)
    {
        if(cut[i] < 0)
        {
...
static  void _sbc_process_8bytes(const uint32 *in, uint32 *out)
{
    ...
    t[7] = SCALE8_STAGE1(
        ...
        //uint32 cut[8];
        cut[0] = SCALE8_STAGE2( (s[0] + s[1]) + (s[2] + s[3]) + s[4]);

        cut[1] = SCALE8_STAGE2( (s[0] - s[1]) + (s[8] - s[9]) + s[5]);

        cut[2] = SCALE8_STAGE2( (s[0] - s[1]) - s[8] + (s[9] + s[6]));

        cut[3] = SCALE8_STAGE2( (s[0] + s[1]) - (s[2] + s[3]) + s[7]);

        cut[4] = SCALE8_STAGE2( (s[0] + s[1]) - (s[2] + s[3]) - s[7]);

        cut[5] = SCALE8_STAGE2( (s[0] - s[1]) - s[8] + (s[9] - s[6]));

        cut[6] = SCALE8_STAGE2( (s[0] - s[1]) + (s[8] - s[9]) - s[5]);

        cut[7] = SCALE8_STAGE2( (s[0] + s[1]) + (s[2] + s[3]) - s[4] );
        for(i = 0;i < 8;i ++)
        {
            if(cut[i] < 0)
            {
                if(cut[i] < -2147483648)
                    out[i] = -32768;
                     ...
                     cut[i] = - cut[i];
                     out[i] = (int32_t)(cut[i] >> 15);
                     out[i] = -out[i];
                }

                //out[i] =(out[i]^0xFFFFFFFF) + 1;
            }
            else
                out[i] =(int32_t)(cut[i] >> 15);
        }
    ...
    if (frame->mode == SBC_MODE_JOINT_STEREO) {
        ...
        for (sb = 0; sb < frame->subbands - 1; sb++) {
            ...
            for (blk = 0; blk < frame->blocks; blk++) {
                ...
                if (sb_sample_j[blk][0] < -32768)
					sb_sample_j[blk][0] = -32768;
				else if (sb_sample_j[blk][0] > 32767)
					sb_sample_j[blk][0] = 32767;

                sb_sample_j[blk][1] =
                    (frame->sb_sample_f[blk][0][sb] -
                        frame->sb_sample_f[blk][1][sb]) >> 1;

                if (sb_sample_j[blk][1] < -32768)
					sb_sample_j[blk][1] = -32768;
				else if (sb_sample_j[blk][1] > 32767)
					sb_sample_j[blk][1] = 32767;
...
static void sbc_virables_init(sbc_t *sbc, unsigned long flags)
{
    ...
    sbc->mode =  sbc_get_sbc_chmode();//SBC_MODE_JOINT_STEREO;  //SBC_MODE_MONO SBC_MODE_STEREO
    sbc->subbands = sbc_get_sbc_subbands();    //8
    sbc->blocks = sbc_get_sbc_block_len();//SBC_BLK_16;
    //sbc->bitpool = 53;           // 53
