...
static void DecodeSectionData(BitStreamInfo *bsi, int winSequence, int numWinGrp, int maxSFB, unsigned char *sfbCodeBook)
{
	...
	for (g = 0; g < numWinGrp; g++) {
		...
		while (sfb < maxSFB) {
			...
			do {
				//By Vincent Hsiung , July 29,2008
				if (timeout_cnt++ > 0x7fff)
					break;

				sectLenIncr = GetBits(bsi, sectLenBits);
				sectLen += sectLenIncr;
				if (raac_getbits_error)
					break;
...
static  unsigned char **gainBits__noiseless = (unsigned char **)(base +gainBits_noiseless_offset );
_ATTR_AACDEC_DATA_RO
static  unsigned char  *bak =(unsigned char *)(base +gainBits_noiseless_offset );
 #else
static  unsigned char gainBits_noiseless[4][3] = {
	{1, 5, 5},  /* long */
	{2, 4, 2},  /* start */
	{8, 2, 2},  /* short */
	{2, 4, 5},  /* stop */
...
static void DecodeGainControlInfo(BitStreamInfo *bsi, int winSequence, GainControlInfo *gi)
{
	...
	#ifdef aac_table_test
	maxWin =      (int)(*(bak+3*winSequence));
	locBitsZero = (int)(*(bak+3*winSequence+1));
	locBits =     (int)(*(bak+3*winSequence+2));
	#else
	maxWin =      (int)gainBits_noiseless[winSequence][0];
	locBitsZero = (int)gainBits_noiseless[winSequence][1];
	locBits =     (int)gainBits_noiseless[winSequence][2];
	#endif
