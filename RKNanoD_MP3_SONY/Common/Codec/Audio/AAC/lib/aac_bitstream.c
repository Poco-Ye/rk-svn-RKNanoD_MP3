...
void RefillBitstreamCache(BitStreamInfo *bsi)
{
	...
	{
		bsi->nBytes = 0;
		raac_getbits_error = 1;
		return;
	}

	raac_getbits_error = 0;
...
void AdvanceBitstreamM(BitStreamInfo *bsi, int nBits)
{
	while(nBits>0)
	{
		if (nBits>31)
		{
			AdvanceBitstream(bsi,31);
/*			
			if (31 > bsi->cachedBits) {
				RefillBitstreamCache(bsi);
			}
			bsi->cachedBits -= 31;
*/			
...
int GetProcBits(BitStreamInfo *bsi)
{
	return CalcBitsUsed(bsi, bsi->startPtr, 0);
}
