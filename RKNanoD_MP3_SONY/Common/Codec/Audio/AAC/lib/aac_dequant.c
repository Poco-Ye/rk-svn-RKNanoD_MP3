...
#define OVERFLOW_MAX      0x7fffffffL
...
static const int * pow14_deq = (int *)(base +pow14_deq_offset);
static const int  **pow43_14_deq =(int **)( base + pow43_14_deq_offset);
static const int *pow43_deq = (int *)(base +pow43_deq_offset);

static const int *poly43lo_deq = (int *)(base +poly43lo_deq_offset);
static const int *poly43hi_deq =(int *)( base +poly43hi_deq_offset);
static const int *pow2exp_deq = (int *)(base +pow2exp_deq_offset);
static const int *pow2frac_deq =(int *)( base +pow2frac_deq_offset);
#else 
 ...
 */
static const int pow43_14_deq[4][16] = {
    {
 ...
 */
static const int poly43lo_deq[5] = { 0x29a0bda9, 0xb02e4828, 0x5957aa1b, 0x236c498d, 0xff581859 };
static const int poly43hi_deq[5] = { 0x10852163, 0xd333f6a4, 0x46e9408b, 0x27c2cef0, 0xfef577b4 };

/* pow2exp[i] = pow(2, i*4/3) exponent */
static const int pow2exp_deq[8]  = { 14, 13, 11, 10, 9, 7, 6, 5 };

/* pow2exp[i] = pow(2, i*4/3) fraction */
static const int pow2frac_deq[8] = {
...
static int DequantBlock(int *inbuf, int nSamps, int scale)
{
    ...
    #ifdef aac_table_test
    tab16 =(const int *)( pow43_14_deq+(scale & 0x3)*16);
    #else
    tab16 = pow43_14_deq[scale & 0x3];
    #endif
    scalef = pow14_deq[scale & 0x3];
    ...
    if (shift > 31) {
        ...
        for (x = 0; x < 4; x++) {
            y = tab16[x];
            if (y > (OVERFLOW_MAX >> shift))
                y = OVERFLOW_MAX;     /* clip (rare), guarantees at least 1 gb */
            else
                y <<= shift;
            tab4[x] = y;
        }
    ...
    do {
        ...
        if (x < 4) {
            ...
            __asm{  SMULL   z,  y, y, scalef    };
                ...
                coef = (x < SQRTHALF) ? poly43lo_deq : poly43hi_deq;
            ...
            __asm{  SMULL   z,  y, y, x };
            y = y + coef[4];
                //y = MULSHIFT32(y, pow2frac_deq[shift]) << 3;
            __asm{  SMULL   z,  y, y, pow2frac_deq[shift]   };
            y = y << 3;
            ...
            __asm{  SMULL   z,  y, y, scalef    };
                shift = 24 - scalei - pow2exp_deq[shift];
            }
