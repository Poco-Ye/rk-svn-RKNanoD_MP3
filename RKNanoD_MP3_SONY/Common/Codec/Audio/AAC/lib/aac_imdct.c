...
static void DecWindowOverlap(int *buf0, int *over0, short *pcm0, int nChans, int winTypeCurr, int winTypePrev)
{
    ...
    if (winTypeCurr == winTypePrev) {
        ...
        do {
            ...
            //f0 = MULSHIFT32(w0, in);
    __asm{  SMULL   f1, f0, w0, in  };
            //f1 = MULSHIFT32(w1, in);
    __asm{  SMULL   in, f1, w1, in  };
            ...
            *pcm1 = CLIPTOSHORT( (in + f1 + RND_VAL) >> FBITS_OUT_IMDCT );
            ...
            in = *buf1--;
    __asm{  SMULL   f1, f0, w0, in  };
            *over1-- = f0;//MULSHIFT32(w0, in);
    __asm{  SMULL   in, f1, w1, in  };
            *over0++ = f1;//MULSHIFT32(w1, in);
        } while (over0 < over1);
    } else {
...
static void DecWindowOverlapLongStart(int *buf0, int *over0, short *pcm0, int nChans, int winTypeCurr, int winTypePrev)
{
    ...
    __asm{  SMULL   f1, f0, w0, in  };
        *over1-- = f0;//MULSHIFT32(w0, in);  /* Wn = short window for n = (1599, 1598, ... , 1536) */
    __asm{  SMULL   in, f1, w1, in  };
        *over0++ = f1;//MULSHIFT32(w1, in);  /* Wn = short window for n = (1472, 1473, ... , 1535) */
    } while (over0 < over1);
}
...
static void DecWindowOverlapShort(int *buf0, int *over0, short *pcm0, int nChans, int winTypeCurr, int winTypePrev)
{
    ...
    __asm{  SMULL   in, f1, w1, in  };
        ...
        *(over1 - 128) = in + f1;
        ...
        in = *buf1--;
    __asm{  SMULL   f1, f0, w0, in  };
        *over1-- = f0;//MULSHIFT32(w0, in);  /* save in overlap[128-191] */
    __asm{  SMULL   in, f1, w1, in  };
        *over0++ = f1;//MULSHIFT32(w1, in);  /* save in overlap[64-127] */
    } while (over0 < over1);
