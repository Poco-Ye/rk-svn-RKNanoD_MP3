...
static  int *nfftTab_fft =(int *)( base +nfftTab_fft_offset);
_ATTR_AACDEC_DATA_RO
static  int *nfftlog2Tab_fft =(int *)( base +nfftlog2Tab_fft_offset);
#else
static  int nfftTab[NUM_FFT_SIZES] =		{64, 512};
static  int nfftlog2Tab[NUM_FFT_SIZES] =	{6, 9};
#endif
...
static void BitReverse(int *inout, int tabidx)
{
	...
	int nbits = nfftlog2Tab_fft[tabidx];
...
static void R8FirstPass(int *x, int bg)
{
	...
	dr = ar - ai;
	__asm{	SMULL	dr,	xr, bi, dr	};
		//xi = MULSHIFT32(SQRT1_2, ar + ai);
	dr = ar + ai;
	__asm{	SMULL	dr,	xi, bi, dr	};
		//zr = MULSHIFT32(SQRT1_2, cr - ci);
	dr = cr - ci;
	__asm{	SMULL	dr,	zr, bi, dr	};
		//zi = MULSHIFT32(SQRT1_2, cr + ci);
	dr = cr + ci;
	__asm{	SMULL	dr,	zi, bi, dr	};
...
static void R4Core(int *x, int bg, int gp, int *wtab)
{
	...
	for (; bg != 0; gp <<= 2, bg >>= 2) {
		...
		for (i = bg; i != 0; i--) {
			...
			//tr = MULSHIFT32(wi, tr);
			__asm{	SMULL	wi,	tr, wi, tr	};
				//br = MULSHIFT32(wd, br) - tr;	/* cos*br + sin*bi */
			//br = MULSHIFT32(wd, br);			/* cos*br + sin*bi */
			__asm{	SMULL	wd,	br, wd, br	};
			br = br - tr;						/* cos*br + sin*bi */
				//bi = MULSHIFT32(ws, bi) + tr;	/* cos*bi - sin*br */
			//bi = MULSHIFT32(ws, bi);			/* cos*bi - sin*br */
			__asm{	SMULL	ws,	bi, ws, bi	};
			bi = bi	+ tr;						/* cos*bi - sin*br */
			...
			//cr = MULSHIFT32(wd, cr);
			__asm{	SMULL	wd,	cr, wd, cr	};
			cr = cr - tr;
				//ci = MULSHIFT32(ws, ci) + tr;
			//ci = MULSHIFT32(ws, ci);
			__asm{	SMULL	ws,	ci, ws, ci	};
			ci = ci	+ tr;
				xptr += step;
			...
			//dr = MULSHIFT32(wd, dr);
			__asm{	SMULL	wd,	dr, wd, dr	};
			dr = dr - tr;
				//di = MULSHIFT32(ws, di) + tr;
			//di = MULSHIFT32(ws, di);
			__asm{	SMULL	ws,	di, ws, di	};
			di = di	+ tr;
				wptr += 6;
...
{
	int order = nfftlog2Tab_fft[tabidx];
	int nfft = nfftTab_fft[tabidx];
