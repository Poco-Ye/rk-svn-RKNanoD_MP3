...
int UnpackADTSHeader(AACDecInfo *aacDecInfo, unsigned char **buf, int *bitOffset, int *bitsAvail)
{
    ...
    if (timeinfo.curframe == 1)
    {
        sameheader = GetBitsNoAdvance(&bsi, 24);
        aac_printf("sameheader=0x%x\n", sameheader);
    }
...
/*
    ...
    fprintf(fp,"frames=%d,len=%x\n",frames,fhADTS->frameLength );
    fclose(fp);
        }
        */
//DEBUG("frames=%d,len=%d\n",frames,fhADTS->frameLength );
    ...
    if (fhADTS->layer != 0 || fhADTS->profile != AAC_PROFILE_LC ||
        ...
        {
         aac_printf("aac_frofile = %d return \n",fhADTS->profile);
        return ERR_AAC_INVALID_ADTS_HEADER;
        }
...
int FlushCodec(AACDecInfo *aacDecInfo)
{
    ...
    psi = (PSInfoBase *)(aacDecInfo->psInfoBase);
    #ifdef GOOD
    ClearBuffer(psi->overlap, 2 * AAC_MAX_NSAMPS * sizeof(int));
    #else
    ClearBuffer(psi->overlap, 3 * AAC_MAX_NSAMPS * sizeof(int));
    #endif
