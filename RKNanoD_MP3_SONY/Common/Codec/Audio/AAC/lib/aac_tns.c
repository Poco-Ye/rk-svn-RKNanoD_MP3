...
static  int *invQuant3 =(int *)( base +invQuant3_offset);
 _ATTR_AACDEC_DATA_RO
 static  int *invQuant4 =(int *)( base +invQuant4_offset);
 #else
static  int invQuant3[16] = {
...
static int FilterRegion(int size, int dir, int order, int *audioCoef, int *a, int *hist)
{
    ...
    do {
        ...
        __asm{SMLAL sum64.r.lo32,sum64.r.hi32, hist[j], a[j]};
            hist[j] = hist[j-1];
        }
        //sum64.w64 = MADD64(sum64.w64, hist[0], a[0]);
    __asm{SMLAL sum64.r.lo32,sum64.r.hi32, hist[0], a[0]};
 ...
 *      DEBUG("static const int invQuant%d[16] = {\n", res);
 *      for (i = 0; i < 16; i++) {
 ...
 *
 *        d *= powScale;
 *        DEBUG("0x%08x, ", (int)(d > 0 ? d + 0.5 : d - 0.5));
 *        if ((i & 0x07) == 0x07)
 *           DEBUG("\n");
 *      }
 *      DEBUG("};\n\n");
