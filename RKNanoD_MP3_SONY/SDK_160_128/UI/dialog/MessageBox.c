...
uint32 MessageBoxKey(void)
{
...
#else   //chad.ma add for process message win key press
    KeyTemp =  GetKeyVal();
    ...
    switch (KeyTemp)
    {
        ...
        case  KEY_VAL_PLAY_SHORT_UP:
            WinDestroy(&MessageBoxWin);
            SendMsg(MSG_MESSAGEBOX_DESTROY);
            SendMsg(MSG_MESSAGEBOX_KEY_ESC);
            break;

        case KEY_VAL_FFW_SHORT_UP:
            WinDestroy(&MessageBoxWin);
            printf("\n MessageBox FFW_SHORT_UP previous song...... \n");
            SendMsg(MSG_MESSAGEBOX_KEY_PREV);
            break;

        case KEY_VAL_FFD_SHORT_UP:
            WinDestroy(&MessageBoxWin);
            printf("\nMessageBox FFD_SHORT_UP next song...... \n");
            SendMsg(MSG_MESSAGEBOX_KEY_NEXT);
            break;

        case KEY_VAL_MENU_SHORT_UP:
            WinDestroy(&MessageBoxWin);
            printf("\nMessageBox MENU_SHORT_UP pause song...... \n");
            SendMsg(MSG_MESSAGEBOX_KEY_MENU);
            break;
...
void MessageBoxDisplay(void)
{
    ...
    if (CheckMsg(MSG_NEED_PAINT_ALL) || GetMsg(MSG_MESSAGE_DISPLAY_ALL))
    {
        ...
        if (TRUE == CheckMsg(MSG_VIDEO_VERTICAL_DISPLAY))
        {
            ...
            DisplayMenuStrWithIDNum(MESSAGE_TEXT_TITLE_X + 16, MESSAGE_TEXT_TITLE_Y - 4,
                                    MESSAGE_TEXT_TITLE_XSIZE, MESSAGE_TEXT_TITLE_YSIZE,
                                    LCD_TEXTALIGN_CENTER, MessageBoxTitleID);      //Title
            DisplayMenuStrWithIDNum(MESSAGE_TEXT_CONTENT_X + 16, MESSAGE_TEXT_CONTENT_Y - 4,
