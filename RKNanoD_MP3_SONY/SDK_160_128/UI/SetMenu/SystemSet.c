...
UINT32 ConvertMemMB(UINT16 *pBuff, UINT32 *Mem)
{
 ...
 /*
--------------------------------------------------------------------------------
  Function name : UINT16 AvlsEnableSet(UINT16 itemNum)
  Author        : Jeff Wu
  Description   :
...
*/
_ATTR_SYS_SET_SYSTEM_CODE_
UINT32 AvlsEnableSet(UINT32 itemNum)
{
    UINT32 retVal = 0;

    SetAvlsEnabled = itemNum;
...
/*
--------------------------------------------------------------------------------
  Function name : UINT16 BassBoostSet(UINT16 itemNum)
  Author        : Jeff Wu
  Description   :
...
*/
_ATTR_SYS_SET_SYSTEM_CODE_
UINT32 BassBoostSet(UINT32 itemNum)
{
    UINT32 retVal = 0;

    SetBassBoost = itemNum;
...
void ProductInfoWinDisplay(void)
{
    ...
    if(GetMsg(MSG_PRODUCTINFO)||(CheckMsg(MSG_NEED_PAINT_ALL)))
    {
        ...
        {
            SyssetScrollInit(&r, NULL, fillDialogBuf, 12);
