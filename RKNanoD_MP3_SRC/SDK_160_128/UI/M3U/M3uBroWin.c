...
void M3uBroVariableInit(void *pArg)
{
    ...
    M3uBroTitle = SID_TEXT_FILE;//SID_M3U_FILE
    if(j)
    {
        M3uDirTreeInfo.M3uCurId[0] = M3uSysFileInfo.BroCurId;
        M3uDirTreeInfo.KeyCounter = M3uSysFileInfo.BroKeyCnt;
    }
}
/*
--------------------------------------------------------------------------------
...
UINT32 M3uBroKey(void)
{
    ...
    //--------------------- FFW KEY --------------------------
        ...
        case KEY_VAL_ESC_SHORT_UP:
            ...
            TaskArg.MainMenu.MenuID = MAINMENU_ID_M3U;
...
UINT32 M3uBroSaveAndPostM3uPlayInfo(void)
{
    ...
    M3uSysFileInfo.BroCurId = M3uDirTreeInfo.M3uCurId[0];
    M3uSysFileInfo.BroKeyCnt = M3uDirTreeInfo.KeyCounter;
        ...
        for(i = 0; ((i < MAX_ITEM_NUM_M3UBRO_DISP) && (i < TotalItem )); i++)
        {
            ...
            if(i == M3uDirTreeInfo.KeyCounter)
            {
                ...
                if(LCD_GetStringSize(pBro->LongFileName) > PicInfo.xSize)
                {
                    ...
                    M3uBroWinScrollInit(&r, IMG_ID_SEL_ICON, pBro->LongFileName, 30);
