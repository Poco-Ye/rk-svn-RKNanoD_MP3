...
void PicBroSortInfoAddrInit(void)
{
    ...
    JpegSortInfoAddr.ulFileSortInfoSectorAddr = MediaInfoAddr + JPEG_TREE_ALL_SORT_INFO_SECTOR_START;
    #endif
}
...
//        }
	...
	gbPicCurid = JpegDirTreeInfo.JpegCurId[0];
	gbPicKeyCnt = JpegDirTreeInfo.KeyCounter;

    #ifdef PIC_MEDIA
	TaskArg.Pic.FsType = JPEG_ALL_DB/*FS_FAT*/;
	TaskArg.Pic.FromWhere = 1;
	TaskArg.Pic.StartFileNum = 0;
    #endif
...
void   PicBroDisplay(void)
{
    ...
    if (GetMsg(MSG_PICBRO_ALL_ITEM))
    {
        ...
        {
#if(defined THUMB_DEC_INCLUDE) && (defined PIC_MEDIA)
            ...
            if (i == JpegDirTreeInfo.KeyCounter)
            {
                ...
                if (LCD_GetStringSize(pBro->LongFileName) > PicInfo.xSize)
                {
                    ...
                    PicBroWinScrollInit(&r, IMG_ID_SEL_ICON, pBro->LongFileName, 30);
