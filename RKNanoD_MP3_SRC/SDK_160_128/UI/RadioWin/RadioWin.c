...
void RadioWinInit(void *pArg)
{
    ...
    else
    {
        ...
        {
            BeepSampleRate_FM = FS_192KHz;
            BeepDatawidth_FM = ACodec_I2S_DATA_WIDTH24;
        }
        else
        {
            BeepSampleRate_FM = FS_48KHz;
...
UINT32 RadioWinService(void)
{
    ...
    if (TRUE == GetMsg(MSG_RADIOSUBWIN_DISPLAY_REC))
    {
        ...
        if (RECORD_QUALITY_HIGH == FM_RecordQuality)  //quality record.
        {
...
//        ModuleOverlay(MODULE_ID_FILE_FIND, MODULE_OVERLAY_ALL);
        ...
        {
            //DEBUG("fmfilenum = %d",gSysConfig.MedialibPara.gRecordFmFileNum);
            #ifdef _MEDIA_MODULE_
            TaskArg.MediaBro.CurId= 0;
            TaskSwitch(TASK_ID_MEDIABRO, &TaskArg);
            RetVal = 1;
...
void RadioWinPaint(void)
{
    ...
    if (TRUE == GetMsg(MSG_RADIOWIN_DISPLAY_BASICFREQ))
    {
        ...
        else    //RadioArea == RADIOAREA_JAPAN
        {
            DisplayBuf[0] = '7';
            DisplayBuf[1] = '6';
            DisplayBuf[2] = 'M';
