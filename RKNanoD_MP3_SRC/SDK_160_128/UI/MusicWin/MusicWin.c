...
void MusicWinPaint(void)
{
    ...
    if (TRUE == GetMsg(MSG_MUSIC_DISPFLAG_CURRENT_TIME)) // current play time
    {
        ...
        if (CurrentTimeSec != CurrentTimeSecBk)
        {
            ...
            {
                //add this judge for some audio format seek not accurate
                {
    ...
    switch (MusicKeyVal)
    {
        ...
        case KEY_VAL_ESC_SHORT_UP:                             //enter brower.
            ...
            else if (AudioFileInfo.ucSelPlayType ==  SORT_TYPE_SEL_M3U_BROWSER)
            {
                ...
                TaskArg.M3u.FromWhere = 1;//FS_FAT type
        ...
        case KEY_VAL_ESC_PRESS_START:                          //退出音乐界面
            ...
            {
//              BeepPlay(BEEP_AMS_RETURN, 1, BEEP_VOL_SET, 1, CodecConfig_Europe[gSysConfig.OutputVolume].BeepFadeDeep);
            }
        ...
        case KEY_VAL_FFD_PRESS:                                 //快进
            ...
            else
            {
                ...
                if ((pAudioRegKey->CurrentTime <= AUDIO_PLAYLEN))
                {
                    MusicDecodeProc(MSG_AUDIO_FF_PAUSE, NULL);
                    MusicDecodeProc(MSG_AUDIO_FFD, (void*)(AUDIO_STEPLEN - AUDIO_PLAYLEN));
                    MusicDecodeProc(MSG_AUDIO_FF_RESUME, NULL);
                    SendMsg(MSG_MUSIC_DISPFLAG_SCHED);
                    MusicStepTime = pAudioRegKey->CurrentTime;
                }
                else if (((pAudioRegKey->CurrentTime - MusicStepTime) >= AUDIO_PLAYLEN) &&
                         (pAudioRegKey->CurrentTime <= MusicTotalTime))
                {
                    ...
                    for (i = 0; i < 200; i++)
                    {
                        ...
                        {
                            MusicStepTime             = MusicTotalTime;
                            pAudioRegKey->CurrentTime = MusicTotalTime;
                        }

                        if (pAudioRegKey->CurrentTime >= MusicStepTime)
                        {
                            MusicStepTime = pAudioRegKey->CurrentTime;
                            break;
                        }
                        ...
                        {
                            if (MusicStepTime - pAudioRegKey->CurrentTime < 500 &&  (MusicTotalTime - pAudioRegKey->CurrentTime) < 500)
                            {
                                pAudioRegKey->CurrentTime = MusicStepTime;
                                break;
                            }
                        }
                ...
                else if (MusicCurrentTime > MusicTotalTime)
                {
                    MusicStepTime             = MusicTotalTime;
                    pAudioRegKey->CurrentTime = MusicTotalTime;
                }
        ...
        case KEY_VAL_FFW_PRESS:                                  //快退
            ...
            else
            {
                ...
                {
                    uint32 temp = pAudioRegKey->CurrentTime;

                    if (pAudioRegKey->CurrentTime > (AUDIO_STEPLEN + AUDIO_PLAYLEN))
                    {
                        MusicStepTime = pAudioRegKey->CurrentTime - (AUDIO_STEPLEN + AUDIO_PLAYLEN);
                    }
                    else
                    {
                        MusicStepTime = 0;
                    ...
                    for (i = 0; i < 200; i++)
                    {
                        ...
                        if ((pAudioRegKey->CurrentTime < temp) && (pAudioRegKey->CurrentTime > MusicStepTime))
                        {
                            MusicStepTime = pAudioRegKey->CurrentTime;
                            break;
                        }
...
#endif
                {
                    ...
                    if (pAudioRegKey->TotalTime > AUDIO_STEPLEN)
                    {
                        MusicStepTime = pAudioRegKey->TotalTime - AUDIO_STEPLEN;
                    }
                    else
                    {
                        MusicStepTime = pAudioRegKey->TotalTime;
                    }
