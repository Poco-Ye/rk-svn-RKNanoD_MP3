...
void UpdateMediaLibray(void)
{
    ...
    //printf("\n search and save music info start......\n");
    SearchAndSaveMusicInfo();

    #ifdef PIC_MEDIA
    SearchAndSaveJpegInfo();
    #endif

    #ifdef _M3U_
    SearchAndSaveM3uInfo();
    #endif
...
//        ModuleOverlay(MODULE_ID_FILE_ENCODE, MODULE_OVERLAY_ALL);
    ...
    */
    ...
    #endif
#ifdef PIC_MEDIA
    else if(LastTaskId == TASK_ID_BROWSER)
    {
...
#ifdef  _M3U_
    else if(LastTaskId == TASK_ID_M3UBRO)
    {
        if (gSysConfig.MedialibPara.gM3uFileNum)
        {
            TaskArg.M3uBro.CurId= 0;
        }
        else
        {
            LastTaskId = TASK_ID_MAINMENU;
