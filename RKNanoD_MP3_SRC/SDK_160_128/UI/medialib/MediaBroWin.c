...
UINT32 SaveAndPostMusicPlayInfo(void)
{
...
#endif
    {
        if (TRUE == ThreadCheck(pMainThread, &MusicThread))
        {
            if ((gMusicTypeSelID != AudioFileInfo.ucSelPlayType)
                || (MusicDirTreeInfo.CurId[MusicDirTreeInfo.MusicDirDeep] != AudioFileInfo.CurrentFileNum - 1)
                || (MusicDirTreeInfo.MusicDirBaseSortId[MusicDirTreeInfo.MusicDirDeep] != AudioFileInfo.uiBaseSortId[MusicDirTreeInfo.MusicDirDeep])
                || (MusicDirTreeInfo.MusicDirDeep != gSysConfig.MediaDirTreeInfo.MusicDirDeep))//不同专辑之间的切换
            {
                ThreadDelete(&pMainThread, &MusicThread);
...
UINT32 MediaBroService(void)
{
    ...
    if (TRUE == GetMsg(MSG_SOUND_PRESSURE_COUNTER_EXPIRED))
    {
        TaskArg.Volume.VolumeAction = VOLUME_SOUND_PRESSURE_COUNTER_EXPIRED;
        //WinCreat(&MediaBroWin, &VolumeWin, &TaskArg);
    }
            ...
            if ( MediaSubData.KeyCounter == 0)
            {
                switch (gMusicTypeSelID)
                {
                    ...
                    case SORT_TYPE_SEL_GENRE:
                        ...
                        //uiBaseId = MusicDirTreeInfo.MusicDirBaseSortId;
                        if (MusicDirTreeInfo.MusicDirDeep == 0)
                        {
