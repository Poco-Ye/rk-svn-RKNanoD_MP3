...
uint16 GetTotalFiles(uint32 DirClus, uint8 *ExtName, FS_TYPE FsType)
{
    ...
    else if(FsType == JPEG_DB)
    {
        if(DirClus != 0xffffffff)
        {
            for(offset = DirClus; ; offset++)
            {
                MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_INFO_SECTOR_START)<<9) + sizeof(FILE_TREE_BASIC)*(UINT32)(offset), sizeof(FILE_TREE_BASIC), (uint8 *)&FileTreeBasic);
            ...
            while (1)
            {
                ...
                if (temp.Name[0] != FILE_DELETED)
                {
                    if ((temp.Attr & (ATTR_DIRECTORY | ATTR_VOLUME_ID)) == 0)   //是文件
                    {
                        ...
                        //DEBUG("file ext : %c %c %c",temp.Name[8],temp.Name[9],temp.Name[10]);
...
#ifdef _RK_CUE_
{
                        ...
                        if(GetFileType(cue, ExtName) != 0xff && FsType == FS_FAT_EX_VOICE)  //Is to search music files
                        {
                                ...
                                && (temp.Name[10] == 'E'))
                            {
                                ...
                                //dumpMemoryShortA((uint16*)(CUEInfo.file), 10, 1);
                                ret = FindFDTInfoLong(&Fdt, DirClus, &Index, (uint16*)(CUEInfo.file));
                                //DEBUG("///////////////FindFDTInfoLong ret = %d",ret);
                                TotalFiles--;
