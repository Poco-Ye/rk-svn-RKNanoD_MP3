...
uint32 GetCurDir(uint32 DirClus, uint32 index, FS_TYPE FsType)
{
    ...
    else if (FsType == JPEG_DB)
    {
        ...
        if (FileTreeBasic.dwNodeFlag == MEDIA_FILE_TYPE_DIR)
        {
            MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_INFO_SECTOR_START + MEDIAINFO_BLOCK_SIZE * 6)<<9) + sizeof(FILE_TREE_EXTEND)*(UINT32)(FileTreeBasic.dwExtendTreeID), 4, (uint8 *)&cluster);
             //printf("son = %d\n", cluster);
        }
        else
        {
...
uint32 ChangeDir(uint16 SubDirIndex, FS_TYPE FsType)
{
    ...
    else if (FsType == JPEG_DB)
    {
        while (1)
        {
            MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_SORT_INFO_SECTOR_START)<<9) + 2 * (UINT32)(CurDirClus + index), 2, (uint8 *)&temp);
            MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_INFO_SECTOR_START)<<9) + sizeof(FILE_TREE_BASIC)*(UINT32)(CurDirClus + temp), sizeof(FILE_TREE_BASIC), (uint8 *)&FileTreeBasic);
            index++;
            if (FileTreeBasic.dwNodeFlag == MEDIA_FILE_TYPE_DIR)
            {
                MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_INFO_SECTOR_START + MEDIAINFO_BLOCK_SIZE * 6)<<9) + sizeof(FILE_TREE_EXTEND)*(UINT32)(FileTreeBasic.dwExtendTreeID), 4, (uint8 *)&cluster);
...
uint16 GetTotalSubDir(uint32 DirClus, FS_TYPE FsType)
{
    ...
    else if (FsType == JPEG_DB)
    {
        if (DirClus != 0xffffffff)
        {
            for (index = DirClus; ; index++)
            {
                MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_TREE_INFO_SECTOR_START)<<9) + sizeof(FILE_TREE_BASIC)*(UINT32)(index), sizeof(FILE_TREE_BASIC), (uint8 *)&FileTreeBasic);
