...
uint8 AddFDTLong(uint32 Clus, FDT *FDTData, uint32 * pIndex, uint16 * FileName)
{
    ...
    while (ret == RETURN_OK)
    {
        ...
        if (ret == RETURN_OK)
        {
            if (TempFDT.Name[0] == FILE_DELETED)
            {
                if (EmpFdtCnt == 0)
                {
                    ...
                    EmpFdtClus = FATGetNextClus(Clus, i / (BootSector.BPB_SecPerClus * 16));  //chad.ma modifyed 20160705
                    EmpFdtIndex = i % (BootSector.BPB_SecPerClus * 16);
                    EmpFdtCnt++;
                }
                else
                {
        ...
        {
            ClusCnt = (LfnCnt - EmpFdtCnt + 1) / (BootSector.BPB_SecPerClus * 16)
                      + ((LfnCnt - EmpFdtCnt + 1) % (BootSector.BPB_SecPerClus * 16))? 1:0;
            ...
            for (k = 0; k < ClusCnt; k++)
            {
                ...
                {
                    ClearClus((uint32)ret);        //2006.11.24 debug by lxs
                    FdtData.DirClus = 0;
                    ret = RETURN_OK;
                    i = 0;
                    EmpFdtCnt = 0;
