 ...
 */
/*
extern unsigned short * mp2_crc_table ;
#define crc_table	mp2_crc_table
*/
extern unsigned short * mp2_crc_table ;
#define crc_table	mp2_crc_table
 ...
 */
_ATTR_MP2DEC_TEXT_
void mp2_mad_bit_init(struct mad_bitptr *bitptr, unsigned char const *byte)
{
 ...
 */
_ATTR_MP2DEC_TEXT_
unsigned int mp2_mad_bit_length(struct mad_bitptr const *begin,
 ...
 */
_ATTR_MP2DEC_TEXT_
unsigned char const *mp2_mad_bit_nextbyte(struct mad_bitptr const *bitptr)
{
 ...
 */
_ATTR_MP2DEC_TEXT_
void mp2_mad_bit_skip(struct mad_bitptr *bitptr, unsigned int len)
{
  bitptr->readbit += len;
}
...
_ATTR_MP2DEC_TEXT_
__asm unsigned long mp2_mad_bit_read(struct mad_bitptr *bitptr, unsigned int len)
{
	PUSH     {r4-r6,lr}
	MOV      r4,r0		;r4 = bitptr
	MOVS     r5,r1		;r5 = len
 ...
 */
_ATTR_MP2DEC_TEXT_
unsigned short mp2_mad_bit_crc(struct mad_bitptr bitptr, unsigned int len,
			   unsigned short init)
{
  ...
  for (crc = init; len >= 32; len -= 32) {
    ...
    data = mp2_mad_bit_read(&bitptr, 32);
  ...
  switch (len / 8) {
  case 3: crc = (crc << 8) ^
	    crc_table[((crc >> 8) ^ mp2_mad_bit_read(&bitptr, 8)) & 0xff];
  case 2: crc = (crc << 8) ^
	    crc_table[((crc >> 8) ^ mp2_mad_bit_read(&bitptr, 8)) & 0xff];
  case 1: crc = (crc << 8) ^
	    crc_table[((crc >> 8) ^ mp2_mad_bit_read(&bitptr, 8)) & 0xff];
  ...
  while (len--) {
    ...
    msb = mp2_mad_bit_read(&bitptr, 1) ^ (crc >> 15);
