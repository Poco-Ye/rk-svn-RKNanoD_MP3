 ...
 * v0.16B3  changed var declarations to , to enforce data align
...
void
idct_int32(short * block, unsigned char *pDst, int stride)
{
    ...
    for (i = 0; i < 8; i++)  /* idct columns */
    {
                 ...
                 | (X5 = blk[8 * 7]) | (X6 = blk[8 * 5]) | (X7 = blk[8 * 3])))
        {
            ...
            blk[8 * 0] = blk[8 * 1] = blk[8 * 2] = blk[8 * 3] = blk[8 * 4] =
                                                       blk[8 * 5] = blk[8 * 6] = blk[8 * 7] =
                                                                                     iclp[(blk[8 * 0] + 32) >> 6];
#else
            pdst[stride * 0] = pdst[stride * 1] = pdst[stride * 2] = pdst[stride * 3] = pdst[stride * 4] =
                                                       pdst[stride * 5] = pdst[stride * 6] = pdst[stride * 7] =
                                                                                     (unsigned char)iclp[(blk[8 * 0] + 32) >> 6];
        ...
        blk[8 * 7] = iclp[(X7 - X1) >> 14];
#else
		pdst[stride * 0] = (unsigned char)iclp[(X7 + X1) >> 14];
        pdst[stride * 1] = (unsigned char)iclp[(X3 + X2) >> 14];
        pdst[stride * 2] = (unsigned char)iclp[(X0 + X4) >> 14];
        pdst[stride * 3] = (unsigned char)iclp[(X8 + X6) >> 14];
        pdst[stride * 4] = (unsigned char)iclp[(X8 - X6) >> 14];
        pdst[stride * 5] = (unsigned char)iclp[(X0 - X4) >> 14];
        pdst[stride * 6] = (unsigned char)iclp[(X3 - X2) >> 14];
        pdst[stride * 7] = (unsigned char)iclp[(X7 - X1) >> 14];
#endif		
    }
