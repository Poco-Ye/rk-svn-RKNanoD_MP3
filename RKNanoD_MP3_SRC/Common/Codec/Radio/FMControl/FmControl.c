...
//               0x00: 0dB; 0xf : -15dB
_ATTR_FMCONTROL_DATA_
FM_SRUCT_CODEC_CONFIG FmCodecConfig_General_700[MAX_VOLUME + 1] =
{
    15, 0, 12,   0, 254, 15,  //  0
    15, 0, 12,   0, 144, 15,  //  1
    15, 0, 12,   0, 140, 15,  //  2
...
_ATTR_FMCONTROL_DATA_
FM_SRUCT_CODEC_CONFIG FmCodecConfig_General_400[MAX_VOLUME + 1] =
{
    15, 0, 12,   0, 254, 15,  //  0
    15, 0, 12,   0, 160, 15,  //  1
    15, 0, 12,   0, 148, 15,  //  2
...
_ATTR_FMCONTROL_DATA_
FM_SRUCT_CODEC_CONFIG FmCodecConfig_Europe[MAX_VOLUME + 1] =
{
    15, 0, 12,   0, 254, 15,  //  0
    15, 0, 12,   0, 160, 15,  //  1
    15, 0, 12,   0, 148, 15,  //  2
...
#else
    memcpy((UINT8*)(&FmDriverTable), (uint8*)(&(ModuleInfo.FMDriverTable.FmDriver[ModuleInfo.FMDriverTable.FMDriverIndex])), sizeof(FM_DRIVER_INFO_T));
#endif
}
...
#ifdef _BEEP_
    if (RECORD_QUALITY_HIGH == FM_RecordQuality)
    {
        BeepSampleRate_FM = ENC_WAV_H_FS;
        BeepDatawidth_FM = ACodec_I2S_DATA_WIDTH24;
    }
    else
    {
        if (FM_RecordFmt == RECORD_FMT_MP3)
        {
            BeepSampleRate_FM = ENC_MP3_N_FS;
        }
        else
        {
            BeepSampleRate_FM = ENC_WAV_N_FS;
        }

        BeepDatawidth_FM = ACodec_I2S_DATA_WIDTH16;
    }

    //DEBUG("---BeepPlayerState = %d", BeepPlayerState);

    if (BeepPlayerState == Voice_PLAY)
    {
#ifndef _BEEP_NOT_MIXED_
        //Codec_SetMode(Codec_Line1ADC,FS_8000Hz);
#else
        BeepStop();
#endif
    }
