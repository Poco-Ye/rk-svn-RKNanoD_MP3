...
int ParseCueInfo(HANDLE fHandle, CUE_INFO * cue_info, uint8 OnlyFile)
{
    ...
    for (i = 0, j = 0; i < iFileSize /*iReadSize*/; ) {
        ...
        if ((iCueComInfo == CUE_TITLE) && (albumflag == 1)) {
            albumflag = 0;
            iCopySize = CUE_GetMetaData(&CUETempBuff[j + 7], iReadSize - j - 7, iCueComInfo,(unsigned char *)cue_info->album, EncodeMode);
            if (iCopySize != 0) {
                j += (7 + iCopySize + 1);
                i += (7 + iCopySize + 1);
            }        
...
READDATA:
    ...
    //for (i = 0; i < cue_info->m_total_Song; i++)
    //{
    //    printf("cue_info->m_songs[%d]: start=%d, end=%d\n", i,cue_info->m_songs[i].start,cue_info->m_songs[i].end);
    //}
    //printf("\n");
