...
_ATTR_BB_SYS_BSS_  short  *WavoutBuf[2];
        ...
        case SUBFN_CODEC_DECODE:
            {
                ...
                switch (pPCM->wFormatTag)
                {
                    case WAVE_FORMAT_ADPCM:
                        {
                            pPCM->OutLength = msadpcm_read_s (&sf, &WavoutBuf[pPCM->CurDecodeIdx][SRC_Num_Forehead] , pPCM->usSamplesPerBlock * pcm_s.ucChannels)
                                              / pcm_s.ucChannels;
                    ...
                    case WAVE_FORMAT_PCM:
                        {
                            ...
                            pOut_24 = (char *)&WavoutBuf[pPCM->CurDecodeIdx][SRC_Num_Forehead];
                            ...
                            else if (pPCM->uBitsPerSample == 32)
                            {
                                ...
                                return 0;
                            //wav_printf("lBlockSize = %d",lBlockSize);
                            ...
                            {
                                 //wav_printf("0:pPCM->uBitsPerSample = %d",pPCM->uBitsPerSample);
                                pPCM->usValid = RKFIO_FRead(pPCM->pucEncodedData, lBlockSize , pRawFileCache );



                                //16 bit quantity
                               // wav_printf("pPCM->uBitsPerSample = %d",pPCM->uBitsPerSample);
                                if (pPCM->uBitsPerSample == 16)
                                {
