...
#include "misc.h"
 ...
 unsigned char stagemasks[10];
 unsigned char stagebooks[10*8];


void res_clear_info(vorbis_info_residue *info){
  if(info){
   
    info->stagemasks = NULL;
	info->stagebooks = NULL;
	ogg_MemSet(stagemasks,0,10);
	ogg_MemSet(stagebooks,0,10*8);
...
int res_unpack(vorbis_info_residue *info,
 ...
 info->stagemasks=stagemasks; 
 info->stagebooks=stagebooks;
...
int res_inverse(vorbis_dsp_state *vd,vorbis_info_residue *info,
  ...
  //char **partword = NULL;
  char partword_used[2][200];//局部变量 /  partword[j]=(char *)_ogg_malloc(partwords*partitions_per_word* sizeof(*partword[j])); 
  if(partwords*partitions_per_word >200) 
  {  
    printf("\n partword_used size %d\n",(partvals+partitions_per_word-1));  
  }
		...
		if(info->type){
		  if(vorbis_book_decodev_add(stagebook,in[j]+offset,&vd->opb,
					     samples_per_partition,-8)==-1)
				{
				  ...
				  *partword = NULL;
				  ogg_MemSet( partword_used,0,2*200);
		    	goto eopbreak;
		  	}
		}else{
