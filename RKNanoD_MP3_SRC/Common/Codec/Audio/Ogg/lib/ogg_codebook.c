...
ogg_uint16_t q_val[100] ={0};//quantvals 相关 目前最大值为49 ok
...
int _ilog(unsigned int v){
  ...
  ogg_uint32_t decpack(long entry,long used_entry,long quantvals,
...
/* 32 bit float (not IEEE; nonnormalized mantissa +
  ...
  ogg_int32_t _float32_unpack(long val,int *point){
...
/* given a list of word lengths, number of used entries, and byte
 ...
 int _make_words(char *l,long n,ogg_uint32_t *r,long quantvals,
		       codebook *b, oggpack_buffer *opb,int maptype){
  long i,j,count=0;
  long top=0;
  ...
  if(n<2){
    ...
    ogg_MemSet(marker,0,sizeof(marker));
  ...
  int _make_decode_table(codebook *s,char *lengthlist,long quantvals,
			      oggpack_buffer *opb,int maptype){
  int i;
  ogg_uint32_t *work; //临时buffer，可以不用RK_ogg_alloca
  //ogg_uint32_t work[1024*12];
  ...
  work=(ogg_uint32_t *)RK_ogg_alloca((s->used_entries*2-2)*sizeof(*work));   
  ...
  if(_make_words(lengthlist,s->entries,work,quantvals,s,opb,maptype))return 1;
    //s->dec_table=(void*)_ogg_malloc((s->used_entries*(s->dec_leafw+1)-2)*			 
                                // s->dec_nodeb);
  s->dec_table=(void*)RK_malloc((s->used_entries*(s->dec_leafw+1)-2)*s->dec_nodeb);
...
int vorbis_book_unpack(oggpack_buffer *opb,codebook *s){
  ...
  long          i,j;
  int           maptype;
 // char         lengthlist[1024*12];
  ogg_MemSet(s,0,sizeof(*s));

  /* make sure alignment is correct */
  if(oggpack_read(opb,24)!=0x564342)goto _eofout;

  /* first the basic parameters */
  s->dim=oggpack_read(opb,16); //码本维数
  s->entries=oggpack_read(opb,24);  //向量size
  ...
  case 0:
    /* unordered */
    lengthlist=(char *)RK_ogg_alloca(sizeof(*lengthlist)*s->entries);
  ...
  case 1:
	...
	//s->q_val=(ogg_uint16_t*)_ogg_malloc(sizeof(ogg_uint16_t)*quantvals);
	s->q_val= q_val;
	...
	/* need quantized values before */
	//printf("codebook.c--L508->quantvals=%d\n",quantvals);
	if(s->q_bits<=8){
 ...
 _eofout:
  ...
  __inline ogg_uint32_t decode_packed_entry_number(codebook *book, 
						      oggpack_buffer *b){
  ...
  if(book->dec_nodeb==1){
	...
	if(chase&0x80UL)break;
      ...
      for(i=0;i<read;i++){
				int bit=(lok>>i)&1;
                int chaseAddbit = chase + bit;
                int next=t[chaseAddbit];
				//int next=t[chase+bit];
				if(next&0x80){
				  //chase= (next<<8) | t[chase+bit+1+(!bit || t[chase]&0x80)];
				  chase= (next<<8) | t[chaseAddbit+1+(!bit || t[chase]&0x80)];
				  break;
				}
				chase=next;
      }
  ...
  case 2:{
    ...
    for(i=0;i<sdim;i++){
      if(s->q_bits<=8)
				...
				(*vtmp++) = ((unsigned char *)(s->q_val))[entry&mask];
      else
				//v[i]=((ogg_uint16_t *)(s->q_val))[entry&mask];
				(*vtmp++) =((ogg_uint16_t *)(s->q_val))[entry&mask];
      entry>>=s->q_pack;
    }
    */
    if(s->q_bits<=8)
        {
            for(i=0;i<sdim;i++){
                (*vtmp++) = ((unsigned char *)(sval))[entry&mask];
                entry>>=spack;
                }
        }
    else
        {
            for(i=0;i<sdim;i++){
                (*vtmp++) =((ogg_uint16_t *)(sval))[entry&mask];
                entry>>=spack;
                }        
        }
    break;
  ...
  case 3:{
    ...
    if(s->q_bits<=8){
      for(i=0;i<sdim;i++)
				...
				(*vtmp++)= *(((unsigned char *)ptr)++);
    }else{
      for(i=0;i<sdim;i++)
				//v[i]=((ogg_uint16_t *)ptr)[i];
				(*vtmp++)= *(((ogg_uint16_t *)ptr)++);
    }
    break;
  }
  default:
  ...
  /* we have the unpacked multiplicands; compute final vals */
  {
    ...
    if(shiftM>0)
    {
      ...
      for(i=0;i<sdim;i++)
				...
				//*(vtmp++) = add + (((*vtmp) * s->q_del) >> shiftM);
				*(vtmp++) = add + (((*vtmp) * sdel) >> shiftM);
		}
    else
      {
      //for(i=0;i<s->dim;i++)
...
long vorbis_book_decodevs_add(codebook *book,ogg_int32_t *a,
  ...
  int step=n/book->dim;
  ogg_int32_t *v = (ogg_int32_t *)RK_ogg_alloca(sizeof(*v)*book->dim);
  int i,j,o;
