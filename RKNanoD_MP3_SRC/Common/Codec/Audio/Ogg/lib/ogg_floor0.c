...
vorbis_info_floor0 info0[2];
int info_floor0_num = 0;
...
static long ADJUST_SQRT2[2]={8192,5792};
 __inline ogg_int32_t vorbis_invsqlook_i(long a,long e){
...
#else
 __inline ogg_int32_t vorbis_fromdBlook_i(long a){
  if(a>0) return 0x7fffffff;
  if(a<(-140<<12)) return 0;
...
/* a is in 0.16 format, where 0==0, 2^^16-1==PI, return 0.14 */
 __inline ogg_int32_t vorbis_coslook_i(long a){
...
/* a is in 0.16 format, where 0==0, 2^^16==PI, return .LSP_FRACBITS */
 __inline ogg_int32_t vorbis_coslook2_i(long a){
  int i=a>>COS_LOOKUP_I_SHIFT;
  int d=a&COS_LOOKUP_I_MASK;
...
static ogg_uint16_t barklook[54]={
...
/* used in init only; interpolate the long way */
 __inline ogg_int32_t toBARK(int n){
  int i;
  for(i=0;i<54;i++)
...
void vorbis_lsp_to_curve(ogg_int32_t *curve,int n,int ln,
  ...
  int ampi=amp;
  ogg_int32_t *ilsp=(ogg_int32_t *)RK_ogg_alloca(m*sizeof(*ilsp));
  ...
  for(i=0;i<m;i++){
    ...
    if(val<0 || (val>>COS_LOOKUP_I_SHIFT)>=COS_LOOKUP_I_SZ){
      ogg_MemSet(curve,0,sizeof(*curve)*n);
      RK_ogg_alloca_exit();
      return;
    }
...
void floor0_free_info(vorbis_info_floor *i){
 ...
 if(info) info = NULL;//_ogg_free(info);
  ogg_MemSet(&info0,0,sizeof(vorbis_info_floor0)*2);
}
...
vorbis_info_floor *floor0_info_unpack (vorbis_info *vi,oggpack_buffer *opb){
  ...
  vorbis_info_floor0 *info= &info0[info_floor0_num++];
...
int floor0_inverse2(vorbis_dsp_state *vd,vorbis_info_floor *i,
  ...
  ogg_MemSet(out,0,sizeof(*out)*ci->blocksizes[vd->W]/2);
  return(0);
}
