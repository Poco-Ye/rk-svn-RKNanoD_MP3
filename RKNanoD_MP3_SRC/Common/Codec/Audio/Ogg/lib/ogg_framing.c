...
ogg_reference or_s[45] = {0};
ogg_buffer ob_s[50] = {0};
//ogg_reference or_s[50] = {0};
//ogg_buffer ob_s[30] = {0};
ogg_buffer_state  buf_st;
ogg_sync_state syn_st;
ogg_stream_state str_st;
...
{
    ogg_buffer_state *bs = &buf_st;
    ogg_MemSet(bs, 0, sizeof(*bs));
    return bs;
}
...
void _ogg_buffer_destroy(ogg_buffer_state *bs)
{
    ...
    if (bs->shutdown)
    {
        ...
        /* //Move down by Vincent Hisung , Dec 23,2007
        if(!bs->outstanding){
          _ogg_free(bs);
          return;
        }
...
//Move to here by Vincent Hisung , Dec 23,2007
        if (!bs->outstanding)
        {
            bs = NULL;
            ogg_MemSet(&buf_st, 0, sizeof(ogg_buffer_state));
        }
    }
}
...
ogg_buffer *_fetch_buffer(ogg_buffer_state *bs, long bytes)
{
    ...
    else
    {
        ...
        ob = &ob_s[ob_s_ind];
        ogg_MemSet(ob, 0, sizeof(ogg_buffer));
        ob_s_ind ++;
...
ogg_reference *_fetch_ref(ogg_buffer_state *bs)
{
    ...
    else
    {
        ...
        or = &or_s[or_s_ind];
        ogg_MemSet( or , 0, sizeof(ogg_reference));
        or_s_ind++;
...
ogg_reference *ogg_buffer_sub(ogg_reference * or , long length)
{
...
void _positionB(oggbyte_buffer *b, int pos)
{
    if (pos < b->pos)
    {
...
void _positionF(oggbyte_buffer *b, int pos)
{
...
int oggbyte_init(oggbyte_buffer *b, ogg_reference * or )
{
    ogg_MemSet(b, 0, sizeof(*b));
...
/*   CRC calculation table.  See older code in CVS for dead
...
// ogg_sync_state *oy=(ogg_sync_state *)_ogg_calloc(1,sizeof(*oy));
    ogg_sync_state *oy = &syn_st;
    ogg_MemSet(oy, 0, sizeof(*oy));
...
int ogg_sync_destroy(ogg_sync_state *oy)
{
    if (oy)
    {
        ...
        //_ogg_free(oy);
        oy = NULL;
        ogg_MemSet(&syn_st, 0, sizeof(ogg_sync_state));
    }
...
int ogg_sync_wrote(ogg_sync_state *oy, long bytes)
{
    ...
    //ogg_DEBUG("fifo_head length= %d",oy->fifo_head->length);
    //ogg_DEBUG("fifo_fill = %d",oy->fifo_fill);
    return OGG_SUCCESS;
}
...
*/
  int header_flag = 0;//cdd
  int page2_header_len = 0;
  int ogg_comment_len = 0;//cdd
  unsigned int ogg_stup_len = 0;
  int fetch_flag = 1;
...
long ogg_sync_pageseek(ogg_sync_state *oy, ogg_page *og)
{
    ...
    bytes = oy->fifo_fill;
    ogg_comment_len = 0;
    //ogg_DEBUG("bytes = %d",bytes);
    oggbyte_init(&page, oy->fifo_tail);
 #ifdef JUMP_COMMENT
   if ((header_flag == 3) )//cdd
    {
    ...
    if (oy->headerbytes == 0)
    {
        ...
        if (fetch_flag)
	    {
		  header_flag = oggbyte_read1(&page, (oy->headerbytes));//cdd,header_flag == 3¡À¨ª¨º?comment?¨¦
	    }
        //ogg_DEBUG("header_flag = %d",header_flag);
   #endif

        //ogg_DEBUG("headerbytes = %d", oy->headerbytes);
    }

    if (bytes < oy->headerbytes)
    {
        //ogg_DEBUG(" bytes = %d headerbytes = %d",bytes,oy->headerbytes);
        goto sync_out; /* not enough for header +
                                             seg table */
    }
  if(oy->bodybytes==0){
	  int i;
#ifdef JUMP_COMMENT//cdd 2016.11.08

//	if (5 != header_flag)
	{
		//int i;
		for (i = 0; i<oy->headerbytes - 27; i++)
			oy->bodybytes += oggbyte_read1(&page, 27 + i);//¡À?¨°3¦Ì?¡Á¨¹3¡è?¨¨( = ????¨ºy¨¢? * ????3¡è?¨¨)
	}
	//else
	if ((3 == header_flag ))//header_flag = 3 comment header
	{
	//	int i;
		int temp = 0;
		for (i = 0; i < oy->headerbytes - 27; i++)
		{
			temp = oggbyte_read1(&page, 27 + i);
			ogg_comment_len += temp;//?y¨¨¡¤?¨®3?comment_len
			if (0xFF != temp)
			{
				ogg_stup_len = oy->bodybytes - ogg_comment_len;
				page2_header_len = oy->headerbytes;
				oy->bodybytes =  ogg_stup_len;
				//oy->headerbytes = 0;
				//oy->fifo_fill = oy->headerbytes + CHUNKSIZE;
				oy->fifo_fill = 0;
                ogg_DEBUG("%d ,%d " ,page2_header_len,ogg_stup_len);
				return header_flag;
			}
		}
	}
	if (ogg_comment_len == oy->bodybytes)
	{

		return ogg_comment_len;
	}
    ...
    //{
      //  int i;
        ...
        //ogg_DEBUG("bodybytes = 0x%x", oy->bodybytes);
    //}
#endif
}
       ...
       checksum */
    {
        ...
        //ogg_DEBUG("chksum = 0x%08x",chksum);
...
{
    ogg_stream_state *os = &str_st;
    ogg_MemSet(os, 0, sizeof(*os));
...
void _next_lace(oggbyte_buffer *ob, ogg_stream_state *os)
{
...
void _span_queued_page(ogg_stream_state *os)
{
...
int _packetout(ogg_stream_state *os, ogg_packet *op, int adv)
{
    ...
    if (-1 == header_flag )
  {
	  os->body_fill = ogg_stup_len | FINFLAG;
      //ogg_DEBUG("%d,%u ",ogg_stup_len,os->body_fill);
	  os->body_fill_next = 0;
	  header_flag = 0;
  }
    #endif
