...
vorbis_info_floor1 info_floor1[2] ={0} ;
int info_floor1_num = 0;
extern int len_dec_table[44];
extern long sum_len_dec[44];
extern signed char  f_open_dec_table;
#if 0
ogg_uint16_t postlist[65];//count+2
char forward_index[65];//count+2
char  loneighbor[63];//count
char  hineighbor[63];//count

char partitionclass[32];//info->partitions
floor1class class_s[5];//maxclass+1
#endif
...
void floor1_free_info(vorbis_info_floor *i){
 ...
 #if 0
 info->class = NULL;
 info->partitionclass = NULL;
 info->postlist = NULL;
 info->forward_index = NULL;
 info->hineighbor=NULL;
 info->loneighbor=NULL;
 memset(class_s,0,5);
 memset(partitionclass,0,32);
 memset(postlist,0,65);
 memset(forward_index,0,65);
 memset(hineighbor,0,63);
 memset(loneighbor,0,63);
 #endif
  #if 1

 ogg_MemSet(info->class,0,5);
 ogg_MemSet(info->partitionclass,0,32);
 ogg_MemSet(info->postlist,0,65);
 ogg_MemSet(info->forward_index,0,65);
 ogg_MemSet(info->hineighbor,0,63);
 ogg_MemSet(info->loneighbor,0,63);
 #endif
  ogg_MemSet(&info_floor1,0,sizeof(*info)*2);
   info = NULL;
	//_ogg_free(info);
...
static int ilog(unsigned int v){
 ...
 void mergesort(char *index,ogg_uint16_t *vals,ogg_uint16_t n){
  ogg_uint16_t i,j;
...
vorbis_info_floor *floor1_info_unpack (vorbis_info *vi,oggpack_buffer *opb){
 ...
 // vorbis_info_floor1 *info=(vorbis_info_floor1 *)_ogg_calloc(1,sizeof(*info));
  vorbis_info_floor1 *info = &info_floor1[info_floor1_num++];
  /* read partitions */
  info->partitions=oggpack_read(opb,5); /* only 0 to 31 legal */

//    info->partitionclass=partitionclass;
...
//   info->class =class_s;
  ...
  info->forward_index= forward_index;
  info->loneighbor= loneighbor;
  info->hineighbor= hineighbor;
   #endif
  count=0;
  for(j=0,k=0;j<info->partitions;j++){
    count+=info->class[info->partitionclass[j]].class_dim; 
 ...
 void render_line(int x0,int x1,int y0,int y1,ogg_int32_t *d){
  int dy=y1-y0;
  int adx=x1-x0;
  int ady=abs(dy);
  int base=dy/adx;
