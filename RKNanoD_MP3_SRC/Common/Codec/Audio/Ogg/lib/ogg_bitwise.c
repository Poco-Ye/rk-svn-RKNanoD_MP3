...
__inline void _span(oggpack_buffer *b){
...
__inline long oggpack_look(oggpack_buffer *b,int bits){
...
__inline void oggpack_adv(oggpack_buffer *b,int bits){
  bits+=b->headbit;
  b->headbit=bits&7;
  b->headend-=(bits>>3);
  b->headptr+=(bits>>3);
...
__inline int oggpack_eop(oggpack_buffer *b){
  if(b->headend<0)return -1;
  return 0;
}

/* bits <= 32 */
__inline long oggpack_read(oggpack_buffer *b,int bits){
  long ret=oggpack_look(b,bits);
  oggpack_adv(b,bits);
  return(ret);
}
#if 0
__inline long oggpack_bytes(oggpack_buffer *b){
  if(b->headend<0)return b->count+b->head->length;
  return b->count + b->head->length-b->headend + 
    (b->headbit+7)/8;
}

__inline long oggpack_bits(oggpack_buffer *b){
