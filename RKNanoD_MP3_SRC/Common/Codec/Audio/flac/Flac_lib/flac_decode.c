...
extern int isRightAfterSeek;
extern  unsigned long SRC_Num_Forehead;
#define flac__utils_printf
...
_ATTR_FLACDEC_BSS_
DecoderSession g_flacblock_decoder_session;

_ATTR_FLACDEC_BSS_
int  g_iFlacStreamDecoderState;// = 0 ;
 ...
 * local routines
 */
static FLAC__bool DecoderSession_construct(DecoderSession *d, FLAC__bool is_ogg,
                                                FLAC__bool is_aiff_out,
                                                FLAC__bool is_wave_out,
                                                FLAC__bool continue_through_decode_errors,
...
static FLAC__bool canonicalize_until_specification(utils__SkipUntilSpecification *spec,
                                                        const char *inbasefilename, unsigned sample_rate,
                                                        FLAC__uint64 skip,
...
_ATTR_FLACDEC_DATA_
static  char flacversion[] = "Version:0.0.1 \nDate:2012.3.23 \nLib:flac_dec_lib" ;
_ATTR_FLACDEC_TEXT_
char * FlacDecVersion()
{
    return flacversion;
...
_ATTR_FLACDEC_TEXT_
FLAC__bool DecoderSession_read_metadata(DecoderSession *d)
{
#ifdef FLAC__HAS_OGG
    if (d->is_ogg)
    {
...
_ATTR_FLACDEC_TEXT_
int Flac_decoder_init()
{
    FLAC__bool analysis_mode = false ; // 是否打印出每帧的基本信息
    analysis_options  aopts;
    wav_decode_options_t options ;
    aopts.do_residual_gnuplot = false ;
    aopts.do_residual_text   = false ;
    options.common.continue_through_decode_errors = false ;
    options.common.replaygain_synthesis_spec.apply = 0 ;
    options.common.replaygain_synthesis_spec.use_album_gain = 1 ;
    options.common.replaygain_synthesis_spec.limiter = 2 ;
    options.common.replaygain_synthesis_spec.noise_shaping = 2 ;
    options.common.replaygain_synthesis_spec.preamp = 0.00 ;
#ifdef FLAC__HAS_OGG
    options.common.is_ogg = 0 ;
    options.common.use_first_serial_number = 1 ;
    options.common.serial_number = 0 ;
#endif
    options.common.skip_specification.is_relative = 0;
    options.common.skip_specification.value_is_samples = 1 ;
    options.common.skip_specification.value.seconds = 0.00 ;
    options.common.skip_specification.value.samples  = 0 ;
    options.common.until_specification.is_relative = 1;
    options.common.until_specification.value_is_samples = 1 ;
    options.common.until_specification.value.seconds = 0.00 ;
    options.common.until_specification.value.samples  = 0 ;
    options.common.has_cue_specification = 0 ;

    //options.common.cue_specification.has_start_point

    if (!DecoderSession_construct(
            &g_flacblock_decoder_session,
#ifdef FLAC__HAS_OGG
    ...
    if (!DecoderSession_init_decoder(&g_flacblock_decoder_session, options.common, NULL))
        //return DecoderSession_finish_error(&g_flacblock_decoder_session);
        return false;

    if (!DecoderSession_read_metadata(&g_flacblock_decoder_session))
        //return DecoderSession_finish_error(&g_flacblock_decoder_session);
        return false;
...
_ATTR_FLACDEC_TEXT_
int Flac_decoder_decode_frame()
{
    DecoderSession   *d = &g_flacblock_decoder_session ;
    FLAC__file_decoder_process_until_end_of_file(d->decoder.flac.file) ;

    if (g_iFlacStreamDecoderState == FLAC__STREAM_DECODER_END_OF_STREAM || g_iFlacStreamDecoderState == FLAC__STREAM_DECODER_ABORTED)
    {
        return g_iFlacStreamDecoderState ;
    }
    else
    {
        return FLAC__STREAM_DECODER_UNPARSEABLE_STREAM ;
    }
}

_ATTR_FLACDEC_TEXT_
int Flac_decoder_finish()
{
    return DecoderSession_finish_ok(&g_flacblock_decoder_session) ;
}
    ...
    /*if(!d->test_only) {
     if(0 == strcmp(outfilename, "-")) {
      ...
      if(0 == (d->fout = flac_fopen(outfilename, "wb"))) {
       ...
       DecoderSession_destroy(d, true);
       return false;
      }
     }
    }*/
    /*
    d->fout = (FILE*)g_hWaveFile;
    if (d->fout == 0)
    {
        DEBUG("Failed to open wave file!\n");
        return false;
    }
    */
#ifdef _DEBUG
...
FLAC__bool DecoderSession_init_decoder(DecoderSession *decoder_session, decode_options_t decode_options, const char *infilename)
{
...
#endif
    {
        ...
        FLAC__ASSERT(decoder_session->cue_specification == 0);
        FLAC__ASSERT(!decoder_session->replaygain.spec.apply);
        //if (0 != decoder_session->cue_specification)
...
/* 此函数中调用printf函数, 汇编中会生成BL       __1fprintf, 其是通过串口打印信息, 需在main中对串口初始化,因此link时会报
 * 找不到main()错误 */
_ATTR_FLACDEC_TEXT_
...
FLAC__StreamDecoderWriteStatus write_callback(const void *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
    ...
    /*
     * BUGBUG:
     * decoder_session->until_specification->value.samples这个值没有被初始化，可能会出问题
     */
    ...
    if (wide_samples > 0)
    {
        ...
        else if (!decoder_session->test_only)
        {
            ...
            {
#if 1
                //DEBUG("We do not support the case decoder_session->replaygain.apply != 0 !\n");
                FLAC__ASSERT(0);
#else
...
extern volatile int g_Flac_Buf_idx;
_ATTR_FLACDEC_BSS_
int flac_output_length;
...
FLAC__StreamDecoderWriteStatus write_callback(const void *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
    ...
    unsigned wide_samples = frame->header.blocksize, wide_sample/*, sample, channel, byte*/;
    size_t bytes_to_write = 0;
    /* 输出BUFFER ,I Can swap them here , by Vincent @ Apr 9 , 2009 */
    FLAC__int16 *s16buffer;// = (FLAC__int16 *)(&g_FlacCodecBuffer[0][0]);
    FLAC__int32 *pbuffer0 = (FLAC__int32*)buffer[0];
    (void)decoder;
    /* swap the ping-pong buffer , @Apr 10 , 2009 */
    s16buffer = (FLAC__int16 *)(&g_FlacCodecBuffer[g_Flac_Buf_idx][SRC_Num_Forehead]);
//#ifdef HALF_FRAME_BY_HALF_FRAME
    wide_samples = frame->header.blocksize >> isNeedDecByHalfFrmae;
...
//#endif
    ...
    if (wide_samples > 0)
    {
...

        {
            ...
            else if (bps == 16)
            {
                ...
                {
                    //s16buffer = (FLAC__int16 *)buffer[0] + 1;
                    FLAC__ASSERT(channels >= 1);
                    FLAC__ASSERT(channels <= 2); // 只支持单声道和双声道的数据
#if 1
                    if (2 == channels)
                    {
                        for (wide_sample = 0; wide_sample < wide_samples; wide_sample++)
                        {
                            *s16buffer++ = (FLAC__int16)buffer[0][wide_sample];
                            *s16buffer++ = (FLAC__int16)buffer[1][wide_sample];
                        }
                    }
                    else if (1 == channels)
                    {
                        for (wide_sample = 0; wide_sample < wide_samples; wide_sample++)
                        {
                            *s16buffer++ = (FLAC__int16)pbuffer0[wide_sample];
                            *s16buffer++ = (FLAC__int16)pbuffer0[wide_sample];
                        }
                    }
#else
                ...
                FLAC__ASSERT(is_big_endian == is_big_endian_host_);
                bytes_to_write = 2 * 2 * wide_samples;
            }
            else
            {
                FLAC__ASSERT(0);
    ...
    /* by Vincent @ Apr 13 , 2009 */
    flac_output_length = wide_samples;

    if (bytes_to_write > 0)
    {
        if (isRightAfterSeek)
        {
            memset(g_FlacCodecBuffer[g_Flac_Buf_idx], 0, bytes_to_write);
        }

        /*Todo: 写输出文件,此处我们应该送CODEC*/
#if 0
        if (flac__utils_fwrite((FLAC__int32*)g_FlacCodecBuffer[g_Flac_Buf_idx], 1, bytes_to_write, fout) != bytes_to_write)
        {
            /* if a pipe closed when writing to stdout, we let it go without an error message */
            /*if(errno == EPIPE && decoder_session->fout == stdout)
             decoder_session->aborting_due_to_until = true;
...
void metadata_callback(const void *decoder, const FLAC__StreamMetadata *metadata, void *client_data)
{
    ...
    if (metadata->type == FLAC__METADATA_TYPE_STREAMINFO)
    {
        ...
        FLAC__ASSERT(decoder_session->bps == 16);
    ...
    {
#if 1
        //DEBUG("We do not support metadata->type == FLAC__METADATA_TYPE_CUESHEET!\n");
        FLAC__ASSERT(0);
#else
    ...
    {
#if 1
        //DEBUG("We do not support metadata->type == FLAC__METADATA_TYPE_VORBIS_COMMENT!\n");
        FLAC__ASSERT(0);
#else
        if (decoder_session->replaygain.spec.apply)
        {
