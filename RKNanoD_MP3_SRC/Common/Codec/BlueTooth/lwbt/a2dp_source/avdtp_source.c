...
_ATTR_LWBT_DATA_ static uint8 a2dp_service_record[] =
{
    ...
    0x35, 0x6, 0x19, 0x0, 0x19, 0x09, 0x1,0x02, /* Protocol descriptor list attribute */
    0x35, 0x0E,                                 /*Attribute: Language Base Attribute ID List*/
    0x09, 0x00, 0x06, 0x35, 0x09, 0x09, 0x65, 0x6e, 0x09, 0x00, 0x6a, 0x09, 0x01, 0x00,
    0x35, 0xD,
    0x9, 0x0, 0x9, 0x35,0x08,0x35, 0x06, 0x19,0x11,0x0d,0x09,0x01,0x03, /* BluetoothProfileDescriptorList */
...
//  0x35, 0x8,
//  0x9, 0x03, 0x11, 0x35,0x03, 0x9, 0x00, 0x01 /* Supported Features attribute */
};
...
static bool avdtp_set_configuration_cmd(struct l2cap_pcb *pcb, struct pbuf *p,struct avdtp_single_header *header)
{
    ...
    configuration_cmd->Int_SEID = (AVDTP_SEP_ID << 2);
    ...
    configuration_cmd->MediaType = a2dp_pcbs->local_sep.Media_Type;
    configuration_cmd->MediaCodecType = AVDTP_AUDIO_CODEC_SBC;
    if(header != NULL)
    {
        configuration_cmd->SF_CM = ((a2dp_pcbs->Localcodec.Sampling_Frequency) | (a2dp_pcbs->Localcodec.Channel_Mode));

        configuration_cmd->BL_BS_AM = ((a2dp_pcbs->Localcodec.Block_Length) |
                                       (a2dp_pcbs->Localcodec.Subbands) |
                                       (a2dp_pcbs->Localcodec.Allocation_Method));
...
static bool avdtp_get_all_capabilities_resp(struct l2cap_pcb *pcb, struct pbuf *p,struct avdtp_single_header *header)
{
    ...
    cap_resp->LOSC1 = 6;
    cap_resp->MediaType = a2dp_pcbs->local_sep.Media_Type;
    cap_resp->MediaCodecType = AVDTP_AUDIO_CODEC_SBC;
    cap_resp->SF_CM = ((a2dp_pcbs->Localcodec.Sampling_Frequency) | (a2dp_pcbs->Localcodec.Channel_Mode));

    cap_resp->BL_BS_AM = ((a2dp_pcbs->Localcodec.Block_Length) |
                          (a2dp_pcbs->Localcodec.Subbands) |
                          (a2dp_pcbs->Localcodec.Allocation_Method));

    cap_resp->MinimumBitpoolValue = a2dp_pcbs->Localcodec.MinimumBitpoolValue;
    cap_resp->MaximumBitpoolValue = a2dp_pcbs->Localcodec.MaximumBitpoolValue;
...
{
    pcbs->local_sep.SEID = SEID;
    pcbs->local_sep.Media_Type = media_type;
    pcbs->local_sep.InUse = InUse;
    pcbs->local_sep.TSEP = TSEP;
    pcbs->Localcodec.Sampling_Frequency = Sampling_Frequency;
    pcbs->Localcodec.Channel_Mode = Channel_Mode;
    pcbs->Localcodec.Block_Length = Block_Length;
    pcbs->Localcodec.Subbands = Subbands;
    pcbs->Localcodec.Allocation_Method = Allocation_Method;
    pcbs->Localcodec.MinimumBitpoolValue = MinimumBitpoolValue;
    pcbs->Localcodec.MaximumBitpoolValue = MaximumBitpoolValue;
    pcbs->Localcodec.delay_reporting = delay_reporting;
...
void avdtp_register(struct a2dp_pcb *pcbs)
{
    ...
    Channel_Mode = (SBC_CHANNEL_MODE_MONO | SBC_CHANNEL_MODE_JOINT_STEREO);
...
static bool avdtp_set_configuration_resp(struct l2cap_pcb *pcb, struct pbuf *p,struct avdtp_single_header *header)
{
    ...
    if(config_a2dpData->Acp_SEID != AVDTP_SEP_ID)
        return FALSE;
...
static bool avdtp_discover_resp(struct l2cap_pcb *pcb, struct pbuf *p,struct avdtp_single_header *header)
{
    ...
    discover_resp_pbuf  = pbuf_alloc(PBUF_RAW, 4, PBUF_RAM);
...
#if 0
_ATTR_LWBT_CODE_
err_t avdtp_recv(void *arg, struct l2cap_pcb *pcb, struct pbuf *s, err_t err)
{
    ...
    switch(a2dphdr->signal_id)
    {
        ...
        case AVDTP_SET_CONFIGURATION:
            //if(avdtp_State_s == AVDTP_STATE_IDLE)
        {
        ...
        case AVDTP_START:
            ...
            if(avdtp_State_s == AVDTP_STATE_SUSPEND)
            {
                avdtp_start_resp(pcb, p,a2dphdr);
                avdtp_State_s = AVDTP_STATE_STREAMING;
                //bt_buf_reset();
        ...
        case AVDTP_CLOSE:
            if((avdtp_State_s == AVDTP_STATE_STREAMING) || (avdtp_State_s == AVDTP_STATE_OPEN))
            {
                ...
                case AVDTP_MSG_TYPE_ACCEPT:
                    avdtp_recv_rsp = 1;
                    {
                        ...
                        while(len)
                        {
                            ...
                            if(MediaType == AVDTP_MEDIA_TYPE_AUDIO && TESP == AVDTP_SEP_TYPE_SINK && in_use == AVDTP_SEPID_NOT_INUSE )
                            {
