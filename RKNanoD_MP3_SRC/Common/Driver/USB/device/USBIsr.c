...
_ATTR_USB_DRIVER_BSS_ uint16 BulkEndpOutSize;
...
/*static*/ void USBEnumDoneIsr(pUDC_DRIVER pUDC)
{
	...
	else if (pDev->ClassType == USB_CLASS_TYPE_AUDIO)
    {
		BulkEndpOutSize = FS_ISOC_RX_SIZE;
    }
...
*/
_ATTR_USB_DRIVER_CODE_
static void USBResetIsr(pUDC_DRIVER pUDC)
{
    uint32 count;
    DEVICE_REG *dev_regs = (DEVICE_REG *)USB_DEV_BASE;    
    USB_OTG_REG *  otg_core = (USB_OTG_REG * )USB_REG_BASE;
