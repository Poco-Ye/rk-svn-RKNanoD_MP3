...
_ATTR_LCDDRIVER_ST7735_CODE_    
 int32 ST7735_DMATranfer (UINT8 x0,UINT8 y0,UINT8 x1,UINT8 y1, UINT16 *pSrc)
 {
    UINT32 size;
    eDMA_CHN channel; 
...
{
#ifdef ILI9163V // second source LCM
    VopSetMode(0, (0x0<<12) | (0x1<<11) | (0x0<<8) | (0x0<<3) | (0x0<<1));
    VopSendCmd(0x00,0x11); //Exit Sleep
    DelayMs(120);

    VopSendCmd(0x00,0x26); //Set Default Gamma
    VopSendData(0x00,0x04);

    VopSendCmd(0x00,0xB1);
    VopSendData(0x00,0x0B);//08
    VopSendData(0x00,0x14);//06

    VopSendCmd(0x00,0xC0); //Set VRH1[4:0] & VC[2:0] for VCI1 & GVDD
    VopSendData(0x00,0x10);//02
    VopSendData(0x00,0x00);

    VopSendCmd(0x00,0xC1); //Set BT[2:0] for AVDD & VCL & VGH & VGL
    VopSendData(0x00,0x03);//02

    VopSendCmd(0x00,0xC5); //Set VMH[6:0] & VML[6:0] for VOMH & VCOML
    VopSendData(0x00,0x39);//4C
    VopSendData(0x00,0x40);//5E

    VopSendCmd(0x00,0xC7);
    VopSendData(0x00,0xB0);//00

    VopSendCmd(0x00,0xEC);
    VopSendData(0x00,0x0C);

    VopSendCmd(0x00,0x3A); //Set Color Format
    VopSendData(0x00,0x05);

    VopSendCmd(0x00,0x2A); //Set Column Address
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x7F);

    VopSendCmd(0x00,0x2B); //Set Page Address
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x00);
    VopSendData(0x00,0x9F);

    VopSendCmd(0x00,0x36); //Set Scanning Direction
    VopSendData(0x00,0xC8);

    VopSendCmd(0x00,0xB7); //Set Source Output Direction
    VopSendData(0x00,0x00);

    VopSendCmd(0x00,0xF2); //Enable Gamma bit
    VopSendData(0x00,0x01);

    VopSendCmd(0x00,0xE0);
    VopSendData(0x00,0x3F);//p1,3B
    VopSendData(0x00,0x25);//p2,2B
    VopSendData(0x00,0x22);//p3,2A
    VopSendData(0x00,0x2F);//p4,2D
    VopSendData(0x00,0x28);//p5,28
    VopSendData(0x00,0x0C);//p6,0C
    VopSendData(0x00,0x4F);//p7,57
    VopSendData(0x00,0xB7);//p8,F0
    VopSendData(0x00,0x3C);//p9,41
    VopSendData(0x00,0x19);//p10,12
    VopSendData(0x00,0x16);//p11,15
    VopSendData(0x00,0x1E);//p12,12
    VopSendData(0x00,0x02);//p13,11
    VopSendData(0x00,0x01);//p14,10
    VopSendData(0x00,0x00);//p15,04
    VopSendCmd(0x00,0xE1);
    VopSendData(0x00,0x00);//p1,04
    VopSendData(0x00,0x1A);//p2,14
    VopSendData(0x00,0x1D);//p3,15
    VopSendData(0x00,0x10);//p4,12
    VopSendData(0x00,0x17);//p5,17
    VopSendData(0x00,0x13);//p6,13
    VopSendData(0x00,0x30);//p7,28
    VopSendData(0x00,0x48);//p8,60
    VopSendData(0x00,0x43);//p9,3E
    VopSendData(0x00,0x06);//p10,0D
    VopSendData(0x00,0x19);//p11,2A
    VopSendData(0x00,0x21);//p12,2D
    VopSendData(0x00,0x3D);//p13,2E
    VopSendData(0x00,0x3E);//p14,2F
    VopSendData(0x00,0x3F);//p15,3B

    VopSendCmd(0x00,0x29); // Display On

    VopSendCmd(0x00,0x2C);
#else
