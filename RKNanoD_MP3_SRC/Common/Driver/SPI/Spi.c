...
void SpiDevIntIsr(void)
{
    uint32 spiIntType, realsize;
    
    spiIntType = SpiGetInt(SPI_CH0);  
    //SPIClearAllInt(SPI_CH0);
...
void SPI_Test(void)
{
    uint8 databuf[32] = {0x55,};
    
    SPI_GPIO_Init(SPI_CH0_PB);
    SPI_HWInit(SPI_CH0);
    SPIInit(SPI_CH0,0, 12*1000*1000, SPI_CTL_TXRX_MASTER | TRANSMIT_ONLY);
    
    IntRegister(INT_ID_SPI0,SpiDevIntIsr);
    ...
    {    
        ...
        SpiEnableChannel(SPI_CH0, 0);
        
        SpiEnableTxInt(SPI_CH0);    
        SPIPioWriteEnable(SPI_CH0,SPI_TRANSMIT_ONLY);
        
        SPIWriteFIFO(SPI_CH0, databuf, 32);
        SpiWaitIdle(SPI_CH0);
        
        SpiDisableChannel(SPI_CH0, 0);
