...
DMA_LLP HIFIllpListn[DMA2_CHN_MAX][2];
...
pFunc HIFI_DmaIsrCallBack[DMA2_CHN_MAX];
...
void HIFI_DmaIsr()
{
    ...
    do
    {
        ...
        do
        {
            for (; channel < DMA2_CHN_MAX; channel++)
            {
        ...
        while (rawStatus & 0x03);

    }
    while ((uint32)(DmaReg2->StatusTfr) & 0x03);
