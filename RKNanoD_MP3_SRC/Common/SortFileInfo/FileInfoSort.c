...
#else
_FILE_INFO_SORT_BSS_    UINT8   SingerBuffer[512]; //2048字节 用于临时记录一个Page的文件信息
_FILE_INFO_SORT_BSS_    UINT8   GerneAblumBuffer[512];   //2048字节 用于临时记录一个Page的文件信息
_FILE_INFO_SORT_CODE_   UINT8   FileIDBuffer[60]; //记录排序文件号
_FILE_INFO_SORT_CODE_   UINT8   FileIDBuffer1[60]; //记录排序文件号
#endif
...
void GetSortNameM3u(UINT32 SectorOffset,UINT16 FileNum,UINT16 Flag)
{
    ...
    for(i=0;i<FileCount;i++)
    {
        if(Flag)
        {
            MDReadData(DataDiskID,((UINT32)(MediaInfoAddr+M3U_SAVE_INFO_SECTOR_START)<<9)+BYTE_NUM_SAVE_PER_FILE*(UINT32)(i)+SectorOffset, SORT_FILENAME_LEN*2, TempBuffer);
            *BkMDReadAdrs = i;
        }
        else
        {
            MDReadData(DataDiskID,((UINT32)(MediaInfoAddr+M3U_SAVE_INFO_SECTOR_START)<<9)+BYTE_NUM_SAVE_PER_FILE*(UINT32)(FileID[i])+SectorOffset, SORT_FILENAME_LEN*2, TempBuffer);
            *BkMDReadAdrs = FileID[i];
        }
...
UINT32 SaveSortInfo(UINT16 Deep, UINT8* SubInfoBuffer,UINT16 StartID,UINT8* FileIDBuf,UINT16 Flag)
{
    ...
    {
#ifdef _JTAG_DEBUG_
        memcpy(FileIDBuf,FileIDBuffer,60);
#else
...
#ifdef _JTAG_DEBUG_
            memset(FileIDBuffer,0,60);
            memset(GerneAblumBuffer,0,512);
            memset(SingerBuffer,0,512);
#else
