...
void PicBroGetMediaItemInfo(UINT16 *pFileName, SORT_INFO_ADDR_STRUCT AddrInfo, UINT16 uiSortId, UINT16 uiCharNum, UINT16 uiCurDeep, UINT16 Flag)
{
    ...
    UINT8   ucBufTemp[8],ucBufTemp1[2];
    UINT32  AddrOffset;
    UINT8 	FileInfoBuf[MEDIA_ID3_SAVE_CHAR_NUM *2];//3
   ...
   else
   {
        ...
        temp1 = (ucBufTemp1[0]&0xff)+((ucBufTemp1[1]&0xff)<<8);
   }

    AddrOffset = (UINT32)(temp1)*BYTE_NUM_SAVE_PER_FILE + AddrInfo.uiSortInfoAddrOffset[uiCurDeep];
...
_ATTR_M3UBROWIN_BSS_ SORTINFO_STRUCT M3uSubinfo;

/*
--------------------------------------------------------------------------------
  Function name : void  M3uBroGetSavedM3uDir(FIND_DATA * pFindData, UINT32 ulFullInfoSectorAddr, UINT32 ulSortSectorAddr, UINT16 uiSortId )
...
*/
_ATTR_SYS_CODE_
void  M3uBroGetSavedM3uDir(FIND_DATA * pFindData, UINT32 ulFullInfoSectorAddr, UINT32 ulSortSectorAddr, UINT16 uiSortId )
{
    UINT32 temp1;
    UINT8 ucBufTemp[2];
    MDReadData(DataDiskID,(ulSortSectorAddr<<9)+((UINT32)uiSortId)*2, 2, ucBufTemp);
...
--------------------------------------------------------------------------------
  Function name :
  void GetM3uItemInfo(UINT16 *pFileName, SORT_INFO_ADDR_STRUCT AddrInfo, UINT16 uiSortId, UINT16 uiCharNum, UINT16 uiCurDeep, UINT16 Flag)
  Author        : anzhiguo
  Description   :
  Input         :
  Return        :
...
void M3uBroGetMediaItemInfo(UINT16 *pFileName, SORT_INFO_ADDR_STRUCT AddrInfo, UINT16 uiSortId, UINT16 uiCharNum, UINT16 uiCurDeep, UINT16 Flag)
{
    UINT16 i;
    UINT16 temp1;
    UINT8   ucBufTemp[8],ucBufTemp1[2];
...
/*
--------------------------------------------------------------------------------
  Function name : UINT16 M3uGetSummaryInfo(UINT32 ulSumSectorAddr, UINT16 uiSumId, UINT16 uiFindSumType)
  Author        : anzhiguo
  Description   :
  Input         :
  Return        :
...
*/
_ATTR_M3UBROWIN_CODE_
UINT16 M3uGetSummaryInfo(UINT32 ulSumSectorAddr, UINT16 uiSumId, UINT16 uiFindSumType)
{
    UINT16 uiSumInfo;
    UINT8  *ucBufTemp;
    UINT32 uiSumTypeOffset;

	ucBufTemp = (UINT8 *)(&M3uSubinfo);

    MDReadData(DataDiskID,(ulSumSectorAddr<<9)+(UINT32)uiSumId*8, 8, ucBufTemp);

    if(uiFindSumType==FIND_SUM_STARTFILEID)
    {
        uiSumInfo = M3uSubinfo.BaseID;
    }
    else if(uiFindSumType==FIND_SUM_SORTSTART)
    {
        uiSumInfo = M3uSubinfo.ItemBaseID;
    }
    else if(uiFindSumType==FIND_SUM_ITEMNUM)
    {
        uiSumInfo = M3uSubinfo.ItemNum;
