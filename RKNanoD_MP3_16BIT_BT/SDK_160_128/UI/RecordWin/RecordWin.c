...
void RecordWinPaint(void)
{
    ...
    if (TRUE == GetMsg(MSG_RECORDWIN_DISPFLAG_TIME))
    {
        ...
        {
            RecordCurrTimerTmb = (uSampleCounter) / ((UINT32)RecordBitrate * 125);   //Bitrate / 8 * 1000 the other arithmetic record
        }
        else if(RecordEncodeType == RECORD_ENCODE_TYPE_PCM)
        {
            if(RECORD_DATAWIDTH_24BIT == RecordDataWidth)
            {
                RecordCurrTimerTmb = uSampleCounter  / ((UINT32)RecordSampleRate * RecordChannel * 3);
            }
            else
            {
                RecordCurrTimerTmb = uSampleCounter / ((UINT32)RecordSampleRate * RecordChannel * 2);
            }
        }
