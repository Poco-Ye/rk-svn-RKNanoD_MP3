...
void M3uWinInit(void *pArg)
{
    ...
    gM3uFsType = FS_FAT;    //Text win use Fs_FAT

	FREQ_EnterModule(FREQ_MEDIAUPDATA);//<----sanshin_20150910
    M3uValueInit(from_where);	//<----sanshin_20150630

	if(from_where == 0)
	{
		//DEBUG("Clus=0x%x, Index=%d\n", M3uSysFileInfo.FindData.Clus, M3uSysFileInfo.FindData.Index);
		...
		{
			/* File Open Error */
			//DEBUG("%s : File Open NG\n", __FUNCTION__);
			TaskArg.Message.TitleID   = SID_WARNING;
        	TaskArg.Message.ContentID = SID_NO_MUSIC_FILE;
        	TaskArg.Message.HoldTime  = 3;
			TaskArg.Message.CurDisFrameIndex = 0;			//<----sanshin_20150629
			TaskArg.Message.UnsupportFrameIndex = 1;		//<----sanshin_20150629
	...
	{
		GetM3uListFileName(gM3uBrowserData.CurStartIndex[0]);	//<----sanshin_20150630
		SendMsg(MSG_M3U_DIS_ALL);
	}
	FREQ_ExitModule(FREQ_MEDIAUPDATA);  //<----sanshin_20150910
...
uint32 M3uWinService(void)
{
	...
	if (CheckMsg(MSG_M3U_NOFIND_FILE))
    {
        if (CheckMsg(MSG_MESSAGEBOX_DESTROY))
        {
        	...
        	TaskArg.M3uBro.CurId= 1;				//<----sanshin_20150630
    ...
    if(GetMsg(MSG_M3U_CHECK_FILE))
    {
        if(gM3uBrowserData.FolderInfo[0].TotalItems==0)
        {
			ClearMsg(M3U_DISPFLAG_SCROLL_FILENAME);
			DispPictureWithIDNum(IMG_ID_BROWSER_BACKGROUND);  //C;SPO`S&8qJ=5DND<~, R2C;SPD?B<, V;OTJ>13>0M<
        }
    }
        ...
        else
        {
            ...
            for(i = 0; (i < M3U_ITEM_PER_PAGE) && (i < TotalItem); i++)
            {
                ...
                //deal with defferent case, the first case is PrePointer , set it as unselect state.
                    ...
                    {
                    //dis play a part(a line for an item) of a big picture. the background picture, is 128 * 160 pixel
                    ...
                    if(i == gM3uBrowserData.CurPointer)
                    {
                        ...
                        if(LCD_GetStringSize(M3uManage.M3u_Music[i].LongFileName) > PicInfo.xSize)
                        {
                            ...
                            M3uWinScrollInit(&r, IMG_ID_SEL_ICON, M3uManage.M3u_Music[i].LongFileName, 3);
...
{
    uint32 i, num;			//<----sanshin_20150630
	uint16 preid, curid;	//<----sanshin_20150630

    //if(NeedInitAll)		//<----sanshin_20150630
    {
...
//---->sanshin_20150630
	if(NeedInitAll)
	{
		//DEBUG("%s preInit : WinStartId=%d, WinCurId=%d, CurrentFileNum=%d\n", __FUNCTION__, M3uSysFileInfo.WinStartId, M3uSysFileInfo.WinCurId, AudioFileInfo.CurrentFileNum - 1);
		...
		//DEBUG("%s postInit : CurStartIndex=%d, CurPointer=%d\n", __FUNCTION__, gM3uBrowserData.CurStartIndex[0], gM3uBrowserData.CurPointer);
	}
	else
	{
		gM3uBrowserData.CurStartIndex[0] = 0;
...
void M3uKeyUpProc(void)
{
	...
	if(CurDirTotalItems == 0)
      ...
      else
      {
        ...
        {
        	if(CurDirTotalItems <= M3U_ITEM_PER_PAGE){							//<----sanshin_20150629
            	gM3uBrowserData.CurStartIndex[0] = 0;
        		gM3uBrowserData.CurPointer = CurDirTotalItems - 1;
        	}else{
        		gM3uBrowserData.CurStartIndex[0] = CurDirTotalItems - M3U_ITEM_PER_PAGE;
        		gM3uBrowserData.CurPointer = M3U_ITEM_PER_PAGE - 1;
        		GetM3uListFileName(gM3uBrowserData.CurStartIndex[0]);			//<----sanshin_20150629
        	}
        }
...
int16 M3uKeyEnterPro(void)
{
	...
	M3uSysFileInfo.WinCurId = gM3uBrowserData.CurPointer;			//<----sanshin_20150630
	M3uSysFileInfo.WinStartId = gM3uBrowserData.CurStartIndex[0];	//<----sanshin_20150630
	...
	//DEBUG("%s : CurPointer = %d, CurStartIndex = %d, SelPoint = %d, M3uGlobalFileCnt = %d\n", __FUNCTION__, gM3uBrowserData.CurPointer, gM3uBrowserData.CurStartIndex[0], temp, AudioFileInfo.M3uGlobalFileCnt);
...
void M3uCreateGlobalNumList(HANDLE handle)
{
   ...
   if (buf[0] == 0xEF && buf[1] == 0xBB && buf[2]== 0xBF)
...

	{
		...
		UINT8  zettai_flag=0;
		UINT8  skip_flag=0;
		UINT8  last_flag=0;		//<----sanshin_20150826
		UINT16 readNum;
		UINT16 remainNum;
		UINT16  charIndex/*, entryItemIndex=0 */;	//<----sanshin_20150917
		char curChar;
		int loop=0, loop1=0;
		...
		do
		{
			...
			//DEBUG("%s : readNum=%d, M3U_TEXT=\n%s\n", __FUNCTION__, readNum, buf);
			remainNum = readNum ;
			charIndex = 0;
			find_flag=0;
			zettai_flag=0;
			...
			/* M3U File Parse Proc     */
			/*-------------------------*/
			//while((remainNum > 0) && (i < M3U_LIST_FILE_MAX))							//<----sanshin_20150826
			while(((remainNum > 0) && (i < M3U_LIST_FILE_MAX) ) || (last_flag == 1))	//<----sanshin_20150826
			{
				if(last_flag == 0)		//<----sanshin_20150826
				{
				remainNum--;
...
//<----sanshin_20150917
				...
				if(find_flag == 1)				// M3U Path Found
				{
					...
					if(M3uPathOffset != 0xFFFF)		// Have a Path
					{
						...
						{
							ch = LineBuf[loop];
							if(/*(UINT8)*/ch == '\\')				// Find Path char		//<----sanshin_20150930
							{
								PathBuf[loop1] = ch;
								//DEBUG("PathBuf[%d]=%c\n", loop1, PathBuf[loop1]);
								path_deep++;
						...
						while((uiFindFileResult == RETURN_OK) && (deep < path_deep))
						{
							...
							{
								memset(M3uLongFileName, 0 , sizeof(M3uLongFileName));	//<----sanshin_20150930
								M3uGetLongFileNameEraseSp(FindDataInfo_Local.Clus, FindDataInfo_Local.Index - 1, FS_FAT_EX_VOICE, M3uLongFileName);	// Get LongFileName		//<----sanshin_20150707

								//DEBUG("M3uLongFileName = ");
								//for(tmp=0; M3uLongFileName[tmp] != 0; tmp++)
								//{
								//	DEBUG("%c", M3uLongFileName[tmp]);
								//}
								//DEBUG("\n");

								judge_flag = 1;
								for(tmp=0; /*(UINT8)*/(PathBuf[(deep * 256)+tmp]) != '\\'; tmp++)		//<----sanshin_20150930
								{
									//DEBUG("M3uLongFileName=%c PathBuf=%c, deep=%d\n", M3uLongFileName[tmp], PathBuf[(deep * 256)+tmp], deep);
					...
					if(find_flag != 0)		// Not Found Path -> No Search		//<----sanshin_20150629
					{
						/* Get FileName */
						...
						while(uiFindFileResult == RETURN_OK)
						{
							...
							if((Fdt.Attr & ATTR_DIRECTORY) == 0)		// Type File
							{
								...
								judge_flag = 1;
								for(tmp=0; /*(UINT8)*/(FileName[tmp]) != '\0'; tmp++)		//<----sanshin_20150930
								{
									//DEBUG("[%3d] M3uLongFileName=0x%04x, FileName=0x%04x\n", tmp, M3uLongFileName[tmp], FileName[tmp]);
								...
								if(M3uLongFileName[tmp] != '\0')	// Check Terminal char
								{
									//DEBUG("LINE=%d, [%3d] : M3uLongFileName=0x%04x, FileName=0x%04x\n", __LINE__, tmp, M3uLongFileName[tmp], FileName[tmp]);
									/* Mismatch LongFileName */
									judge_flag = 0;
								}
							}
						...
						if(Clus != BAD_CLUS)
						{
							//DEBUG("%s : Fdt.Name=%s, Clus=%d\n", __FUNCTION__, Fdt.Name, FindDataInfo_Local.Clus);
							...
							if(FileNum != 0)
							{
								...
								//DEBUG("%s : ////////TEST  globalFileNum[%d]=%d\n", __FUNCTION__, i, globalFileNum);//hoshi
							}
							else
							{
								/* File Not Found */
		...
		//DEBUG("%s : gM3uBrowserData.FolderInfo[0].TotalItems=%d\n", __FUNCTION__, gM3uBrowserData.FolderInfo[0].TotalItems);
	}
}
...
int16 M3uGetFileNumInFloderByShortName(UINT8 *shortName, UINT32 DirClus , FileType FileType)
{
    ...
    while (1)
    {
		...
		//DEBUG_PrintShortName("GetFDTName = ",TempFDT.Name);
        ...
        if (TempFDT.Name[0]!=FILE_DELETED)
        {
            ...
            if (!( TempFDT.Attr & ATTR_DIRECTORY ))//file
            {
                if( M3uCheckFileType( TempFDT.Name, FileType ))
                {
					...
					//DEBUG_PrintShortName("GetFDTName = ",TempFDT.Name) ;
					//DEBUG_PrintShortNameCodeNum("GetFDTName = ",TempFDT.Name) ;
...
void GetM3uListFileName(UINT16 start)
{
	...
	//DEBUG("num=%d, ucSelPlayType=%d\n", num, AudioFileInfo.ucSelPlayType);
	...
	for(i=0; i < num; i++)
	{
    	...
    	//DEBUG("%s : gM3uGlobalFileNumBuf=0x%x, Clus = %d, Index = %d\n", __FUNCTION__, gM3uGlobalFileNumBuf[start + i], m3uFileInfo.FindData.Clus, m3uFileInfo.FindData.Index);
    	GetLongFileName(m3uFileInfo.FindData.Clus, m3uFileInfo.FindData.Index - 1, FS_FAT, M3uManage.M3u_Music[i].LongFileName);

		//{
    	//	for(loop=0; M3uManage.M3u_Music[i].LongFileName[loop] != '\0' ;loop++){
    	//		printf("%c", M3uManage.M3u_Music[i].LongFileName[loop]);
    	//	}
    	//	printf("\n");
    	//}
	}
...
*
*  Input:   DirClus          両両 DirClus
*           Index            両両 DirIndex
*           FsType           両両 FsType
*  Output:  lfName           両両 LongFileName
*  Return:
*  Calls:
*
*  History:     <author>         <time>         <version>
*             Sanshin          2015/07/07         Ver1.0
*     desc: ORG
********************************************************************************
*/
_ATTR_M3U_UI_CODE_
void M3uGetLongFileNameEraseSp(int32 DirClus, int32 Index, FS_TYPE FsType, uint16* lfName )
{
    ...
    {                                                                                                                                                                                                       /*<--sanshin 0612*/
        ...
        MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_SAVE_INFO_SECTOR_START)<<9)+BYTE_NUM_SAVE_PER_FILE*(UINT32)(FileTreeBasic.dwBasicInfoID)+ DIR_CLUS_SAVE_ADDR_OFFSET, 4, (uint8 *)&(DirClus));  /*<--sanshin 0612*/
        MDReadData(DataDiskID,((UINT32)(MediaInfoAddr + JPEG_SAVE_INFO_SECTOR_START)<<9)+BYTE_NUM_SAVE_PER_FILE*(UINT32)(FileTreeBasic.dwBasicInfoID)+ DIR_INDEX_SAVE_ADDR_OFFSET, 4, (uint8 *)&(Index));   /*<--sanshin 0612*/
        Index--;                                                                                                                                                                                            /*<--sanshin 0612*/
    }                                                                                                                                                                                                       /*<--sanshin 0612*/
#endif																																																		/*<--sanshin 0612*/
    memset((void*)lfName, 0, (MAX_FILENAME_LEN * 2));
    GetFDTInfo(&TempFDT, DirClus, Index);//嬁曽徾，搊嬐暸壒暸奀椔廃挍
//------------------------------------------------------------------------------
//榞媌媆嬍椔廃挍
    for (i=0; i<8; i++)
    {
...
//斿崨歰奀椔廃挍，帒敓奀椔廃挍
    ...
    while (Item <= MAX_LFN_ENTRIES)
    {
        uint16 * plfName;
        uint8 *buf;
        GetFDTInfo(&TempFDT, DirClus, Index - Item);//嬁曽徾，搊嬐暸壒暸奀椔廃挍
