...
uint32 IsrDisable(UINT32 ulInterrupt)
{
    ...
    else if (ulInterrupt >= 16)
    {
        ...
        //printf("\n irq0 = 0x%8x, irq1 = 0x%08x", nvic->Irq.Enable[0], nvic->Irq.Enable[1]);
    }

    //printf("\n ulInterrupt = %d, state = 0x%08x", ulInterrupt, IsEnable);
...
--------------------------------------------------------------------------------
  Function name :
  Author        : ZHengYongzhi
  Description   : Enable and Disable user ints, for app critical protection
