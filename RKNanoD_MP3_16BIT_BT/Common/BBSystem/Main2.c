...
#ifndef CODEC_24BIT
                        //just for sony task request to wav format
                        if(CurrentCodec2 == CODEC_WAV_DEC)
                        {
                            if((gMediaBlockInfo.SampleRate == 44100 || gMediaBlockInfo.SampleRate == 48000)
                                && (gMediaBlockInfo.Channel == 1 || gMediaBlockInfo.Channel == 2)
                                && (gMediaBlockInfo.Bps == 24 || gMediaBlockInfo.Bps == 32 ))
                            {
                                bb_printf1("### Don't support this according to sony request!###");
                                MailBoxWriteB2ACmd(MEDIA_MSGBOX_CMD_DEC_OPEN_ERR, MAILBOX_ID_0, MAILBOX_CHANNEL_1);
                                MailBoxWriteB2AData(0, MAILBOX_ID_0, MAILBOX_CHANNEL_1);
                                break;
                            }
...
//------------------------------------------------------------------------------
            case RECORD_ENCORD_INIT:
#if(defined WAV_DEC_INCLUDE2) || (defined MP3_ENC_INCLUDE2)
                {
                    ...
                    //bb_printf1(" --- gRecordBlockInfo.encodeType123 ---  = %d",gRecordBlockInfo.encodeType);
                    ...
                    switch( gRecordBlockInfo.encodeType )
                    {
                        ...
                        case RECORD_ENCODE_TYPE_MP3:
                        {
                            ...
                            {
                                NSLen = Get_NS_Init_len(gRecordBlockInfo.sampleRate,gRecordBlockInfo.channel,gRecordBlockInfo.encodeType);
                                //bb_printf1(" --- RECORD_ENCODE_TYPE_MP3 NSLen %d--- ",NSLen);
                                NS_init(NSLen,gRecordBlockInfo.sampleRate);
                            }
                            ...
                            //bb_printf1(" --- sampleRate = %d--- ",gRecordBlockInfo.sampleRate);
                            //bb_printf1(" --- channel = %d--- ",gRecordBlockInfo.channel);
                            //bb_printf1(" --- Bitrate = %d--- ",gRecordBlockInfo.Bitrate);
                            RecordBufferInit();
                            //pMP3_EncodeOutputBuffer = (UINT8 *)(&MP3_EncodeOutputBuffer[0]);
