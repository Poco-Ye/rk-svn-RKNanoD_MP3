    ...
    while (pos <= Id3v2size)                            /* ID3v2末端まで検索 */
    {
        ...
        if (strncmp(fid, "PIC", 3) == 0)
        {
            ...
            break;
                    ...
                    && (strncmp(fid, "TRD", 3) != 0)
                    && (strncmp(fid, "TRK", 3) != 0)
                    && (strncmp(fid, "TSC", 3) != 0)//sanshin_20150914 frame ID for iTunes
                    && (strncmp(fid, "TSI", 3) != 0)
                    && (strncmp(fid, "TSS", 3) != 0)
                    && (strncmp(fid, "TYE", 3) != 0)
                    && (strncmp(fid, "TXX", 3) != 0)
...
static int atomsearchpicdata(FILE *fp)
{
    ...
    if(ftypframe.Header.Length < ATOMHOFS)      //<----sanshin_20151207
    {                                           //<----sanshin_20151207
        return -1;                              //<----sanshin_20151207
    }                                           //<----sanshin_20151207
...
#else
    max_size = FileGetSize((HANDLE)(fp));       //<----sanshin_20151125
...
//---->sanshin_20151205
    ...
    while (totalsize < atomsize)
    {
        ...
        //totalsize += itunesframe.Header.Length;   //<----sanshin_20151207
        totalsize += atomframe->Header.Length;      //<----sanshin_20151207
    }
#else
//<----sanshin_20151205
    int res = -1;
...
//---->sanshin_20151205
    ...
    while (totalsize < atomsize)
    {
        ...
        //if(itunesframe.Header.Length <= 0)        //<----sanshin_20151207
        if(itunesframe.Header.Length < ITH2DOFS)    //<----sanshin_20151207
        {
            break;
        }
