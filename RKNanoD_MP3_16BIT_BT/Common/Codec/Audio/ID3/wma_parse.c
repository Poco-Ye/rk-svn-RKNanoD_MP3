...
#define LOAD_DWORD(p) ((long)((p)[0]) | ((long)((p)[1])<<8) | ((long)((p)[2])<<16) | ((long)((p)[3])<<24) )
#define LOAD_WORD(p)  ((short)((p)[0]) | ((short)((p)[1])<<8))
...
//cdd 20161212
unsigned short WM_TitleSortOrder[18] =
{
    'W','M','/','T','i','t','l','e','S','o','r','t','O','r','d','e','r','\0'
};

unsigned short WM_ArtistSortOrder[19] =
{
    'W','M','/','A','r','t','i','s','t','S','o','r','t','O','r','d','e','r','\0'
};

unsigned short WM_AlbumSortOrder[18] =
{
    'W','M','/','A','l','b','u','m','S','o','r','t','O','r','d','e','r','\0'
};

unsigned long AP_FileRead(char *buffer, long cbLen, AP_FILE fhandle)
{
    unsigned long ret; 

    ret = RKFIO_FRead(buffer, cbLen , (FILE *)fhandle);
...
void FillByteStream(sByteStreamCtl *pByteStreamCtl)
{
	...
	if(cbRead != (unsigned long)pByteStreamCtl->cbOffset)
	{
...
int WMA_LoadExtendedContentDesc_Object(long *pcbConsumed, long currPacketOffset, long cbObject, ID3 *id3)
{
    ...
    for (i = 0; i < cDescriptors; i++)
    {
		...
		int fGenreFind = 0;
        int fTrackFind = 0;
        int fTitleSortFind = 0;
        int fArtistSortFind = 0;
        int fAlbumSortFind = 0;
        ...
        // title reading//20161212
        else if(0 == memcmp(WM_TitleSortOrder, ByteStreamGetBuf(&ByteStreamCtl, 0), sizeof(WM_TitleSortOrder)))
        {
            fTitleSortFind = 1;

        }
        //artist reading
        else if(0 == memcmp(WM_ArtistSortOrder, ByteStreamGetBuf(&ByteStreamCtl, 0), sizeof(WM_ArtistSortOrder)))
        {
            fArtistSortFind = 1;

        }
        //album reading
        else if(0 == memcmp(WM_AlbumSortOrder, ByteStreamGetBuf(&ByteStreamCtl, 0), sizeof(WM_AlbumSortOrder)))
        {
            fAlbumSortFind = 1;
        ...
        if (ECD_STRING == data_type)
        {
            ...
            else if(fGenreFind)
            {
             ...
             else if(fTitleSortFind)//cdd titlereading
            {
                int i;
                cbWanted = cbValue > (MEDIA_ID3_SAVE_CHAR_NUM * 2) ? (MEDIA_ID3_SAVE_CHAR_NUM * 2):cbValue;
                memcpy(id3->TitleReading, ByteStreamGetBuf(&ByteStreamCtl, 0), cbWanted);
                cbWanted >>= 1; // char to wchar, so /2
                //for(i = 0;i<cbWanted-1;i++)
                  // DEBUG("%d ",id3->TitleReading[i]);
                id3->TitleReading[cbWanted-1] = 0; // null
                //DEBUG("@@@@@@@@@@@@@@@@@@@@@");
                count ++;
            }
             else if(fArtistSortFind)//cdd artistreading
            {
                int i;
                cbWanted = cbValue > (MEDIA_ID3_SAVE_CHAR_NUM * 2) ? (MEDIA_ID3_SAVE_CHAR_NUM * 2):cbValue;
                memcpy(id3->AuthorReading, ByteStreamGetBuf(&ByteStreamCtl, 0), cbWanted);
                cbWanted >>= 1; // char to wchar, so /2
                //for(i = 0;i<cbWanted-1;i++)
                    //DEBUG("%d ",id3->TitleReading[i]);
                id3->AuthorReading[cbWanted-1] = 0; // null
                //DEBUG("@@@@@@@@@@@@@@@@@@@@@");
                count ++;
            }
             else if(fAlbumSortFind)//cdd albumreading
            {
                int i;
                cbWanted = cbValue > (MEDIA_ID3_SAVE_CHAR_NUM * 2) ? (MEDIA_ID3_SAVE_CHAR_NUM * 2):cbValue;
                memcpy(id3->AlbumReading, ByteStreamGetBuf(&ByteStreamCtl, 0), cbWanted);
                cbWanted >>= 1; // char to wchar, so /2
                //for(i = 0;i<cbWanted-1;i++)
                //    DEBUG("%d ",id3->TitleReading[i]);
                id3->AlbumReading[cbWanted-1] = 0; // null
                //DEBUG("@@@@@@@@@@@@@@@@@@@@@");
                count ++;
            }
...
int WMA_ParseAsf_Header(AP_FILE fhandle,  ID3 *id3)
{
    ...
    while((uint32)currPacketOffset < (uint32)cbHeader)
    {
		if(ContentOK >= 2)
		{
