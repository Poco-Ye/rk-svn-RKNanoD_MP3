...
int FadeIsFinished()
{
    ...
    fade=&Fade_types;
    return (fade->is_l_finished & fade->is_r_finished & (fade->is_finished == 4));
}
...
void FadeProcess(fade_short *pwBuffer, unsigned short frameLen)
{
    ...
    for(i = 0; i < frameLen; i++)
    {
        ...
        {
            if((Fade_types.l_value == 0) ||(Fade_types.l_value == -1) )
            {
                if(effl == 1)
                {
            ...
            if((Fade_types.r_value == 0) || (Fade_types.r_value == -1))
            {
                if(effr == 1)
                {
