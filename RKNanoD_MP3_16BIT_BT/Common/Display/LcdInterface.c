...
__align(4)
//static UINT16 frame_buffer[LCD_MAX_YSIZE][LCD_MAX_XSIZE];
/*static*/ UINT16 frame_buffer[BUFFER_MAX_NUM][LCD_HEIGHT][LCD_WIDTH];
...
_ATTR_LCD_CODE_
int32 Lcd_DMATranfer (UINT8 x0,UINT8 y0,UINT8 x1,UINT8 y1,UINT16 *pSrc)
{
    UINT32 size;
    eDMA_CHN channel;
...
*/
_ATTR_LCD_CODE_
int32 Lcd_DMATranfer32 (UINT8 x0,UINT8 y0,UINT8 x1,UINT8 y1,UINT32 *pSrc)
{
    UINT32 size;
    UINT32 *pDst;
    eDMA_CHN channel;
...
void Lcd_Init(void)
{
    ...
    ImageMemSet(frame_buffer, 0 , sizeof(frame_buffer)/(sizeof(uint32) * 8));
    Ymin = 0;
    Ymax = 0;
    CurrentFrameIndex = 0;
    Refresh = 0;
...
void Lcd_BuferTranfer (void)
{
    ...
    if(IsBackLightOn == 1)  //add by chad.ma 20151102,if backlight is off,return directly.
        return ;
...
#if 0   //单buffer
        ret = SystemLcdPara.pLcd_DMATranfer(0, Ymin, LCD_MAX_XSIZE - 1, Ymax - 1,
                                            &frame_buffer[Ymin][0]);
#else  //three buffer

        ret = SystemLcdPara.pLcd_DMATranfer(0, Ymin, LCD_WIDTH - 1, Ymax,
                                            &frame_buffer[CurrentFrameIndex][Ymin][0]);
#endif
...
int LCD_Copy_FrameBuffer(UINT16 src_index,UINT16 dest_index)
{
    ...
    memcpy(&frame_buffer[dest_index][0][0],
           &frame_buffer[src_index][0][0], LCD_WIDTH*LCD_HEIGHT*2);
...
--------------------------------------------------------------------------------
  ...
  Author        : norton from sanshin shenzhen
